...
class MainWindow:
    ...
    def make_implane(self):
        ...
        self.freq = float(self.freq_field.get_text())
        
        #model = self.mode_combo.get_model()
        index = self.mode_combo.get_active()
        if   index == 0:
            self.mode = 'basic'
        ...
        cnu = 3.0
        msun = float(self.bsun_field.get_text())

        model = self.units_combo.get_model()
        index = self.units_combo.get_active()
        if   model[index][0] == 'Kelvin': units = 'K'
        elif model[index][0] == 'Jansky': units = 'Jy'

        ## scatter = False
        ## if(self.scatter.get_active()):
        ##     scatter = True
        scatter = self.scatter_button.get_active() # True if depressed
    ...
    def tbr(self, widget, data=None):
        ...
        title(r'Sun Image in $T_B$ $('+units+')$ at '+
              '%g' % (freq/1e6) + ' MHz')
        #fig.suptitle('Tbr, frequency = '+self.freq_field.get_text())
        show()
    ...
    def tbriquv(self, widget, data=None):
        ...
        title('Sun Image in $T_B$ $('+units+')$ at '+
              '%g' % (freq/1e6) + ' MHz')
        #fig.suptitle('TbrI, frequency = '+self.freq_field.get_text())
        show()
        ...
        title('Sun Image in $T_{BV}$ (Stokes V, $'+units+'$) at '+
              '%g' % (freq/1e6) + ' MHz')
        #fig.suptitle('TbrV, frequency = '+self.freq_field.get_text())
        show()
    ...
    def trajectory(self, widget, data=None):
        ...
        for trj in self.traj.pos[:]:
            ix5 = where(sqrt(trj[:,0]**2 + trj[:,1]**2 + trj[:,2]**2) < 5.0)[0]
        ...
        #ax.text(0.4, 0.96, 'Ray Trajectories at '+self.freq_field.get_text(),
        #        color='k')
        #fig.text(0.4, 0.96, 'Ray Trajectories at '+self.freq_field.get_text()+
        #         ' MHz')
        fig.suptitle('Ray Trajectories at '+self.freq_field.get_text()+' MHz')
    ...
    def makeBatch(self, widget, data=None):
        ...
        sphere = float(self.sphere_field.get_text())
        index = self.mode_combo.get_active()
        if   index == 0: self.mode = 'basic'
        elif index == 1: self.mode = 'Tbr' 
        elif index == 2: self.mode = 'TbrIQUV' 
        else:                      self.mode = ''
    ...
    def __init__(self):
        ...
        self.big_box.set_size_request(350,400)


#makes the spinbuttons for the implane coordinates

        self.implane_box_1 = gtk.HBox(False,10) # Label "Im. Pl. Coords, Rsun"
        self.implane_box_2 = gtk.HBox(False,0)  # Container for table1

        # 1st row of implane coordinates (label)
        self.implane_label = \
            gtk.Label("Image Plane")
        ...
        # Create table and add it to self.implane_box_2
#        self.table1 = gtk.Table(3, 5, True) # 2 rows x 3 columns
        self.table1 = gtk.Table(3, 5, False) # 2 rows x 3 columns
        
        # 1st row of implane table (X extent)
        self.implane_label_X = gtk.Label("X:")
        self.implane_label_X.show()
        self.table1.attach(self.implane_label_X, 0, 1, 0, 1)
        self.implane_spinner_xMin = gtk.SpinButton(
            gtk.Adjustment(-2,-100,100,.1,1),.1,2)
        self.implane_spinner_xMin.show()
        self.implane_spinner_xMin.connect("value-changed",self.paramsChange)
        self.table1.attach(self.implane_spinner_xMin, 1, 2, 0, 1)        
        self.implane_label_to = gtk.Label("-")
        self.implane_label_to.show()
        self.table1.attach(self.implane_label_to, 2, 3, 0, 1)
        self.implane_spinner_xMax = gtk.SpinButton(
            gtk.Adjustment(2,-100,100,.1,1),.1,2)
        self.implane_spinner_xMax.show()
        self.implane_spinner_xMax.connect("value-changed",self.paramsChange)
        self.table1.attach(self.implane_spinner_xMax, 3, 4, 0, 1)        
        self.implane_label_Rsun = gtk.Label("R_sun")
        self.implane_label_Rsun.show()
        self.table1.attach(self.implane_label_Rsun, 4, 5, 0, 1)

        # 2nd row of implane table (Y extent)
        self.implane_label_Y = gtk.Label("Y:")
        self.implane_label_Y.show()
        self.table1.attach(self.implane_label_Y, 0, 1, 1, 2)
        self.implane_spinner_yMin = gtk.SpinButton(
            gtk.Adjustment(-2,-100,100,.1,1),.1,2)
        self.implane_spinner_yMin.show()
        self.implane_spinner_yMin.connect("value-changed",self.paramsChange)
        self.table1.attach(self.implane_spinner_yMin, 1, 2, 1, 2)        
        self.implane_label_to = gtk.Label("-")
        self.implane_label_to.show()
        self.table1.attach(self.implane_label_to, 2, 3, 1, 2)
        self.implane_spinner_yMax = gtk.SpinButton(
            gtk.Adjustment(2,-100,100,.1,1),.1,2)
        self.implane_spinner_yMax.show()
        self.implane_spinner_yMax.connect("value-changed",self.paramsChange)
        self.table1.attach(self.implane_spinner_yMax, 3, 4, 1, 2)        
        self.implane_label_Rsun = gtk.Label("R_sun")
        self.implane_label_Rsun.show()
        self.table1.attach(self.implane_label_Rsun, 4, 5, 1, 2)

        # 3rd row of implane table (grid sizes)
        self.grid_box_1 = gtk.HBox(False,10)
        self.grid_label = gtk.Label("Grid X:")
        self.grid_label.show()
        self.grid_box_1.pack_start(self.grid_label)

        self.grid_spinner_width = gtk.SpinButton(
            gtk.Adjustment(20,0,1000,1,1),1,0)
        self.grid_spinner_width.show()
        self.grid_spinner_width.connect("value-changed",self.paramsChange)
        self.grid_box_1.pack_start(self.grid_spinner_width)

        self.grid_label_height = gtk.Label("Y:")
        self.grid_label_height.show()
        self.grid_box_1.pack_start(self.grid_label_height)
        
        self.grid_spinner_height = gtk.SpinButton(
            gtk.Adjustment(20,0,1000,1,1),1,0)
        self.grid_spinner_height.show()
        self.grid_spinner_height.connect("value-changed",self.paramsChange)
        self.grid_box_1.pack_start(self.grid_spinner_height)
        self.grid_box_1.show()

        self.table1.attach(self.grid_box_1, 0, 4, 2, 3)

        self.grid_label_pixels = gtk.Label("pix")
        self.grid_label_pixels.show()
        self.table1.attach(self.grid_label_pixels, 4, 5, 2, 3)
        self.table1.show()

        
        self.implane_box_2.pack_start(self.table1)
        self.implane_box_2.show()
        self.big_box.pack_start(self.implane_box_2)

#-----------------------------------------------------------------

        self.obssph_box = gtk.HBox(False,10)
        self.observer_label = gtk.Label("Obs:")
        self.observer_label.set_alignment(1,.5)
        self.observer_label.show()

        self.obssph_box.pack_start(self.observer_label)
        
        self.observer_field = gtk.Entry(100)
        self.observer_field.set_text("215.0, 0.0, 0.0")
        self.observer_field.set_size_request(130,25)
        self.observer_field.show()
        
        self.obssph_box.pack_start(self.observer_field)

#makes the integration sphere text

        self.sphere_label = gtk.Label("R_int:")
        self.sphere_label.set_alignment(1,.5)
        self.sphere_label.show()

        self.obssph_box.pack_start(self.sphere_label)
        
        self.sphere_field = gtk.Entry(100)
        self.sphere_field.set_size_request(100,25)
        self.sphere_field.set_text("25.0")
        self.sphere_field.show()
        
        self.obssph_box.pack_start(self.sphere_field)
        
        self.obssph_box.show()
        self.big_box.pack_start(self.obssph_box)

        
#makes the frequency text box
        self.freqbsun_box = gtk.HBox(False,10)

        self.freq_label = gtk.Label("Freq:")
        self.freq_label.set_alignment(1,.5)
        self.freq_label.show()

        self.freqbsun_box.pack_start(self.freq_label)
        
        self.freq_field = gtk.Entry(100)
        self.freq_field.set_size_request(100,25)
        self.freq_field.set_text("80e6")
        self.freq_field.show()
        
        self.freqbsun_box.pack_start(self.freq_field)

        self.freqbsun_box.show()

        self.bsun_label = gtk.Label("B_sun:")
        self.bsun_label.set_alignment(1,.5)
        self.bsun_label.show()

        self.freqbsun_box.pack_start(self.bsun_label)
        
        self.bsun_field = gtk.Entry(100)
        self.bsun_field.set_size_request(100,25)
        self.bsun_field.set_text("1.0")
        self.bsun_field.show()
        
        self.freqbsun_box.pack_start(self.bsun_field)

        self.freqbsun_box.show()
        self.big_box.pack_start(self.freqbsun_box)
        
        #Makes the mode & units como box

        self.modeunits_box = gtk.HBox(False,10)

        self.mode_label = gtk.Label("Mode:")
        self.mode_label.show()
        self.modeunits_box.pack_start(self.mode_label)
        
        self.mode_combo = gtk.combo_box_new_text()
        self.mode_combo.set_size_request(100,25)
        self.mode_combo.append_text('Rays only')
        self.mode_combo.append_text('I Image')
        self.mode_combo.append_text('I,V Image')
        self.mode_combo.set_active(1) # Active "T_br" entry
        self.mode_combo.show()
        self.modeunits_box.pack_start(self.mode_combo)

        # Make Units K/Jy combo
        
        self.units_label = gtk.Label('Units:')
        self.units_label.show()
        self.modeunits_box.pack_start(self.units_label)
        
        self.units_combo = gtk.combo_box_new_text()
        self.units_combo.append_text('Kelvin')
        self.units_combo.append_text('Jansky')
        self.units_combo.set_active(0) # Active "Kelvin" entry
        self.units_combo.show()
        self.modeunits_box.pack_start(self.units_combo)

        self.modeunits_box.show()
        
        self.big_box.pack_start(self.modeunits_box)

        # Make buttons "Scatter" and "Recompile"
        self.scarec_box = gtk.HBox(False,10)

        self.scatter_button = gtk.ToggleButton("Scattering")
        self.scatter_button.show()
        self.scarec_box.pack_start(self.scatter_button)
        ...
        self.scarec_box.pack_start(self.recompile_button)
        
        self.scarec_box.show()
        self.big_box.pack_start(self.scarec_box)
...
#makes the trace button
        ...
        self.graph_button_tbriquv = gtk.Button("Tbr Stokes Graph")
    ...
    def graph(self):
        ...
        if(self.trace.mode>1):
            ...
            if(self.trace.mode==2):
                fig = figure(1)
                clf()
                fig.suptitle(r'Step Mode $T_B$ at ' + freq_str)
            ...
            if(self.trace.mode==3):
                fig = figure(1)
                clf()
                fig.suptitle(r'Step Mode $T_{BI}$ (Stokes I) at ' + freq_str)
                ...
                fig = figure(2)
                clf()
                fig.suptitle(r'Step Mode $T_{BV}$ (Stokes V) at ' + freq_str)
        ...
        plot([-1,1], [0,0], [0,0], 'w') # Invisible line cut to set X bounds
        # Get actual ranges of the X, Y, and Z axes
        xls = ax.get_xlim3d(); xl = xls[1] - xls[0]
        yls = ax.get_ylim3d(); yl = yls[1] - yls[0]
        zls = ax.get_zlim3d(); zl = zls[1] - zls[0]
        rr = sqrt(xl**2 + yl**2 + zl**2)

        #figtext(0.01, 0.02, 'xl=%g, '%xl + 'yl=%g, '%yl + 'zl=%g '%zl +
        #        'rr=%g, '%rr )
        #figtext(0.01, 0.05, '%g, '%(xl/rr) + '%g, '%(yl/rr) + '%g '%(zl/rr))
        
        u = np.linspace(0, 2 * np.pi, 60)
        v = np.linspace(0, np.pi, 60)

        x = xl/yl * np.outer(np.cos(u), np.sin(v))
        y = 1. * np.outer(np.sin(u), np.sin(v))
        z = 1. * np.outer(np.ones(np.size(u)), np.cos(v))
        ...
        fig.suptitle('Trajectories at ' + freq_str)
