...
void advance_beam(
  ...
  double PosBeforeBoris_D[3];  // Position vector before Boris step 
  ...
  for (iRay=start; iRay < stop; iRay++) {
    ...
    if(scattering)
      scatter(Rho_I[iRay],GradRho_ID[iRay],DS_I[iRay],prm,Dir_ID[iRay],iRay);
  ...
  for (iRay = start; iRay < stop; iRay++) {
	...
	|| (DS_I[iRay] < (prm->AbsMinStep))) {
      ...
      /*
       * Increment the ray arc length by approximated parabola length
       */
      Arclen_I[iRay] += LPrb;
	...
	if (SolDist > (prm->r_chro_cor))
      ...
      if (rtmode == 2){ /* Calculate the brightness temperature, Tbr */
	Tbr_I[iRay] = calc_tbr(Param_I, Rho_I[iRay], Te, LPrb, 
			       Tbr_I[iRay], &dtau);
 	OpDepth_I[iRay] += dtau;
      } 
      else if(rtmode==3){ /* Calculate Stokes I, Q, U, and V components of 
			   * brightness temperature                        */
	//printf("LPrb = %g, iRay = %d\n", LPrb, iRay);
	calc_tbriquv(Param_I, &Dir_ID[iRay][0], Rho_I[iRay], 
		     &Bfield_ID[iRay][0], Te, LPrb, 
		     &TbrIQUV_IP[iRay][0], &dtau, &cos_th, thread_num);
 	OpDepth_I[iRay] += dtau;
	CosBD_I[iRay] = cos_th;
      }
      ...
      PosBeforeBoris_D[0] = Pos_ID[iRay][0];
      PosBeforeBoris_D[1] = Pos_ID[iRay][1];
      PosBeforeBoris_D[2] = Pos_ID[iRay][2];
      ...
      /*
       * Increment the ray arc length by the step length
       */
      Arclen_I[iRay] += DS_I[iRay];
	...
	if (SolDist > (prm->r_chro_cor))
      ...
      if (rtmode == 2){ /* Calculate the brightness temperature, Tbr */
	Tbr_I[iRay] = calc_tbr(Param_I, Rho_I[iRay], Te, DS_I[iRay], 
			       Tbr_I[iRay], &dtau);
 	OpDepth_I[iRay] += dtau;
      } 
      else if(rtmode==3) { /* Calculate Stokes I, Q, U, and V components of 
	...
	/*   printf("\n"); */
	/*   printf("In adv_beam: \nBx = %g, By = %g, Bz = %g, B = %g\n", */
	/* 	 Bfield_ID[iRay][0], Bfield_ID[iRay][1], Bfield_ID[iRay][2], */
	/* 	 sqrt(pow(Bfield_ID[iRay][0],2) + pow(Bfield_ID[iRay][1],2) + */
	/* 	      pow(Bfield_ID[iRay][2],2))); */
	/* } */

	calc_tbriquv(Param_I, &Dir_ID[iRay][0], Rho_I[iRay], 
		     &Bfield_ID[iRay][0], Te, DS_I[iRay], 
		     &TbrIQUV_IP[iRay][0], &dtau, &cos_th, thread_num);
	CosBD_I[iRay] = cos_th;
	/* printf("Returned:TbrIQUV_IP = %e %e %e %e\n",  */
	/*       TbrIQUV_IP[iRay][0], TbrIQUV_IP[iRay][1], */
	/*       TbrIQUV_IP[iRay][2], TbrIQUV_IP[iRay][3]); */
 	/* OpDepth_I[iRay] += dtau; */
...
inline double normal_random(double mean, double sigma){
  ...
  while(ret >= 1){
    random1 = random()/(float)RAND_MAX;
    random2 = random()/(float)RAND_MAX;  
    V1 = random1*2.0-1.0;
    V2 = random2*2.0-1.0;
...
void scatter(double Rho,
  ...
  Rho = Rho/prm->ProtonMass_g;//ElectronMass_g/2;
  plasma_freq = 8980*sqrt(Rho);

  mu = sqrt(1.0-pow(plasma_freq,2)/pow(prm->Freq,2));//sqrt(1.0 - Rho*prm->RhoCrInv);

  li = pow(.08,.3333333);
  l0 = pow(.08*1000000,.6666666);

  //b = ((pi*pow(plasma_freq,4)*ep2)/
  //     (pow(prm->Freq,4)*pow(mu,4)*li*l0));
  b = .00003927*pow(plasma_freq,4)/(pow(prm->Freq,4)*pow(mu,4));

  sigma = mu*sqrt(b*DS*prm->Rsun_km);

  sum+=normal_random(0,sigma);
  num++;


  /*
  if(iray==50){
    fprintf(stderr,"mu: %g,plama %g, freq %g, rand %g, Avg %g\n",mu,plasma_freq,prm->Freq,normal_random(0,sigma),(sum/num));
    }*/
  /*
  if(plasma_freq/prm->Freq>.01)
    fprintf(stderr,"iRay: %d ratio %g\n",plasma_freq/prm->Freq);
*/
  ...
  Dir_I[0] += power*normal_random(0,sigma);
  Dir_I[1] += power*normal_random(0,sigma);
  Dir_I[2] += power*normal_random(0,sigma);
