...
PyObject *trace_prdiagn_thread(void *thread_args[]);
...
PyObject *trace_beam(PyObject *self, PyObject *args) {
  ...
  struct BeamData bd = 
    {NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, 
     ...
     thnactive,  &barr,      &toTrace,   0.,   0,    0,    0,    
     0,    0,    0,    0};
...
PyObject *trace_beam_thread(void *thread_args[]){
  ...
  printf("pr_interval = %d, thread = %d\n", pr_interval, thread_num);
  ...
  * because of a rounding error */
  ...
  for (iter = 0; iter < bd->nIter; iter++) {
    ...
    if (iter%pr_interval == 0) {
      printf("Waiting for mutex, thread %d\n", thread_num);
      /* printf("Waiting for barrier, thread %d\n", thread_num); */
      pthread_barrier_wait(bd->barr);
      /* pthread_mutex_lock(&mutex);         /\* lock mutex           *\/ */
      /* while (!predicate) {                    /\* check predicate      *\/ */
      /* 	pthread_cond_wait(&condvar,&mutex);   /\* recheck pred *\/ */
      /* } */
    ...
    /*
     * Every pr_interval wait until all the threads hit the barrier
     */
    // printf("pr_interval = %d, thread = %d\n", pr_interval, thread_num);
...
PyObject *trace_prdiagn_thread(void *thread_args[]){
  ...
  bd->thnactive[thread_num] = 0; /* This thread has no rays */
  ...
  while  (1) {
    ...
    /*
     * Wait until all the threads hit the barrier
     */
    printf("Waiting for barrier in prdiagn, thread %d\n", thread_num);
    pthread_barrier_wait(bd->barr);
    ...
    percent = (double)nactive/(double)bd->nRay;

    printf("DIAG: %7i: =====(%5i) %8.4f%% ====== Min solar Dist: %g\n",
  	   icnt, nactive, 100.0*percent, minsd);

    /*
     * Initialize the barrier
     */
    printf("Destroy barrier in prdiagn, thread # = %d\n", thread_num);
    pthread_barrier_destroy(bd->barr);
    printf("Re-initialize barrier in prdiagn, # of active threads = %d\n", 
	   bd->nacthr);
    pthread_barrier_init(bd->barr, NULL, bd->nacthr);
  } /* while (1) {
