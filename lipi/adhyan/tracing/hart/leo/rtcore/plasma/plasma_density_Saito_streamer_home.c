...
void plasma_density(int nRay, double Pos_ID[nRay][3], 
                    double Rho_I[nRay], 
  ...
  for (iRay = 0; iRay < nRay; iRay++) {
    ...
    if (0) { // Ten times Ne in gramms cm^-3
      ...
      GradRho_ID[iRay][0] = 10.0*Nex*cProtonMass;
      GradRho_ID[iRay][1] = 10.0*Ney*cProtonMass;
      GradRho_ID[iRay][2] = 10.0*Nez*cProtonMass;
    }
    else if (1) { // Just Ne in gramms cm^-3
      Rho_I[iRay] = Ne*cProtonMass;
      GradRho_ID[iRay][0] = Nex*cProtonMass;
      GradRho_ID[iRay][1] = Ney*cProtonMass;
      GradRho_ID[iRay][2] = Nez*cProtonMass;
    }
...
double density_only(double x, double y, double z) {
  ...
  double extern const R0;  /*  = 6.955e5;   // km, Solar radius */
