...
def get_freq(MSNAME):			# Return the start frequency and channel width of the first spectral channel in the data set
	...
	spec_resol = tb.getcol('CHAN_WIDTH')[0][0]/(10**3)
	tb.close()
...
#############################################################################################################################################
INDATA_DIR = '/media/rohit/MWA/20190405_PSP_MWA/sun_1238472032/226MHz/'		# Path of directory where MS files are sitting
OUTDATA_DIR = '/media/rohit/MWA/20190405_PSP_MWA/sun_1238472032/226MHz/'	# Path of directory to which the output DS files are directed
DB_DIR	= '/media/rohit/MWA/20190405_PSP_MWA/sun_1238472032/226MHz/'		# Path of directory where database of MWA events is located
WORKING_DIR = '/media/rohit/MWA/20190405_PSP_MWA/sun_1238472032/226MHz/'		# Path of working directory
...
BASEURL = 'http://mwa-metadata01.pawsey.org.au/metadata/'	# Append the service name to this base URL, eg 'con', 'obs', etc.
database = 'MWA_events.db'					#Name of database
tablename = 'MWA_events'				# Name of table. 
...
if(MWA_PHASE==2):
	...
	tile1=[62,60,25,0,16]
	tile2=[63,61,27,7,17]
        baseline_list=['62-63', '60-61', '25-27', '0-7', '16-17']

os.chdir(INDATA_DIR)
MS_LIST = sorted(glob.glob('1238472032_177-177.ms'))
...
for i in range(len(MS_LIST)):
	os.chdir(INDATA_DIR)	
	MSNAME = MS_LIST[i]
	int_time,start_time,end_time = datetime(MSNAME)
	RefFreq,spec_resol = get_freq(MSNAME)
	#obs_id=MSNAME.split('.')[0]
        obs_id=MSNAME.split('-')[0]
        print "Observation id : \t",obs_id
        #obsinfo = getmeta(BASEURL,service='obs', params={'obs_id':obs_id})
        azi_pointing,ele_pointing,gridpoint_number = 11.31,52.81,81#get_metafit1(obs_id)
        ph_ra,ph_dec=azi_pointing,ele_pointing#get_RADec(MSNAME)
	for j in range(len(baseline_list)):		
		print "Autocorrelations"
		auto_t1[i][j] = get_amp(MS_LIST[i],tile1[j],tile1[j],POL_LIST)			# Autocorrelation from tile 1 - XX and YY pol.
		auto_t2[i][j] = get_amp(MS_LIST[i],tile2[j],tile2[j],POL_LIST)			# Autocorrelation from tile 2	
		print "Crosscorrelations.... size: ", auto_t2[i][j].shape
		cross_t12[i][j] = get_amp(MS_LIST[i],tile1[j],tile2[j],CPOL_LIST)	# Crosscorrelations - XX,XY,YX and YY pols. in sequence
		phase_ncross[i][j],u[i][j],v[i][j],w[i][j] = get_phases(MS_LIST[i],tile1[j],tile2[j],CPOL_LIST)
		print "MSNAME :",MSNAME,"\nTile 1 :",tile1[j],"\nTile 2 :",tile2[j]
