...
MOMMS='/media/rohit/MWA/20190405_PSP_MWA/sun_1238472032/240MHz/selfcal/1238472032_188-188.1.ms' 
#MOMMS='/media/rohit/MWA/20190408_PSP_MWA/1238731768_sun/080MHz/1238731768_062-062.fixvis.ms' 
split_=1
if(split_==1):
    ...
    timerange_='04:03:00~04:03:10'
    #timerange_='04:10:00~04:10:10'
    datacolumn_='data'
...
#phase_centre='J2000 16h25m47.66 -19d40m2.89' #
#solar_centre='';phase_centre='J2000 00h54m37.50 +05d50m36.2'
solar_centre='';phase_centre='J2000 01h06m15.387231s +09d46m47.60506s'
masks_=['']
#phase_centre='J2000 02h04m51.211 12d43m12.47'
#initial_model_L_band='blank_079MHz.im'
initial_model_L_band='blank.im'
...
interactive=False
timerange_=''
modes=['p','p','a','p','p','p','p','p','p','p','a','p','p','p','p']#['p','p','p','p']#['ap','ap','ap','ap','ap','ap','ap']
#modes=['p']*15
#snr_=[2,3,3,3,3,3,3]
snr_=[3]*len(modes)
...
#niter_base=np.array([5,5,5,20,20,20,50,50,50,60,60,60,80,80,80])#[,600,800,1000,1200,1300,1000,1000])
niter_base=np.linspace(100,120,len(modes))#np.ones(len(modes))*50
#niter_base=np.linspace(5,100,len(modes))#np.ones(len(modes))*50
#uvrange_=['2000~7000lambda','500~7000lambda','','','','','','','','']
uvrange_=[0]*len(modes)
start_uv=np.linspace(100,1500,len(modes))#[::-1]
#start_uv=np.linspace(0,80,len(modes))[::-1]
#start_uv=np.logspace(np.log10(2), np.log10(500), num=len(modes))[::-1]
for i in range(len(modes)):
    #uvrange_[i]=str(int(start_uv[i]))+'~2000lambda'
    uvrange_[i]='10~'+str(int(start_uv[i]))+'lambda'
