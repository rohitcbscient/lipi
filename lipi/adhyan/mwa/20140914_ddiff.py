...
(
    ...
    pa_angle,
    pa_amp,
) = pickle.load(open("/media/rohit/MWA/20140914/Tb_centroid.p", "rb"))
...
linex = np.arange(3100, 3500)
liney = np.arange(1270, 1670)[::-1]
mapp = Map(ddiff_list[0])
d = mapp.data
p2w = mapp.pixel_to_world(linex * u.pix, liney * u.pix)
linex_arcsec = p2w.Tx.value
liney_arcsec = p2w.Ty.value
...
for i in range(len(ddiff211_list)):
    mapp = Map(ddiff211_list[i])
    d = mapp.data
    intd211[i] = [0] * len(linex)
...
for i in range(len(ddiff094_list)):
    mapp = Map(ddiff094_list[i])
    d = mapp.data
    intd094[i] = [0] * len(linex)
...
linex_pix = w2p.x.value
liney_pix = w2p.y.value
...
im = ax0.imshow(
    intd171[:, ::-1], aspect="auto", origin="lower", vmin=-60, vmax=60, cmap="coolwarm"
)
ax0.set_xticks(np.arange(intd171.shape[1])[::200])
ax0.set_xticklabels(time171[::200])
ax0.set_yticks(np.arange(400)[::50])
ax0.set_yticklabels(np.round(liner_arcsec[::50], 1))
# ax0.plot(225+np.arange(len(Tbsubmax[0]))[80:],liner_pix[-1][80:],'o-',color='yellow',linewidth=0.5,markersize=2)
ax1 = ax0.twinx()
ax1.plot(
    158 + np.arange(len(Tbsubmax[0]))[80:],
    rcsub90[-1][80:],
    "o-",
    color="k",
    linewidth=1,
    markersize=2,
    label="240 MHz Type-IV radial location",
)
...
ax0.plot(
    ...
    color="cyan",
    linewidth=3,
    label='Slope: -0.03"/s $\\approx$ -21 km/s',
)
ax0.plot(
    speedupx,
    speedupy,
    ...
    color="lime",
    linewidth=3,
    label='Slope: 0.006"/s $\\approx$ 4.3 km/s',
)
ax0.plot(
    speedupx1,
    speedupy2,
    ...
    color="yellow",
    linewidth=3,
    label='Slope: 0.11"/s $\\approx$ 81 km/s',
)
...
ax0.legend(loc=1)
ax1.legend(loc=3)
ax0.set_ylabel("Radial Coordinate (arcsec)")
ax0.set_xlabel("Time (HH_MM_SS UT)")
e1 = Ellipse(xy=(240, 315), width=100, height=70, edgecolor="magenta", fc="None", lw=2)
ax0.add_patch(e1)
e1 = Ellipse(xy=(205, 250), width=80, height=50, edgecolor="magenta", fc="None", lw=2)
ax0.add_patch(e1)
plt.show()
...
f, ax0 = plt.subplots(1, 1)
im = ax0.imshow(
    intd094[:, ::-1], aspect="auto", origin="lower", vmin=-1, vmax=1, cmap="coolwarm"
)
ax0.set_xticks(np.arange(intd094.shape[1])[::200])
ax0.set_xticklabels(time094[::200])
ax0.set_yticks(np.arange(400)[::50])
ax0.set_yticklabels(np.round(liner_arcsec[::50], 1))
# ax0.plot(225+np.arange(len(Tbsubmax[0]))[80:],liner_pix[-1][80:],'o-',color='yellow',linewidth=0.5,markersize=2)
ax1 = ax0.twinx()
...
ax0.plot(
    ...
    color="cyan",
    linewidth=3,
    label='Slope: 0.08"/s $\\approx$ 60 km/s',
)
ax0.plot(
    speed94_2x,
    speed94_2y,
    ...
    color="magenta",
    linewidth=3,
    label='Slope: 0.04"/s $\\approx$ 30 km/s',
)
ax0.plot(
    speedupx,
    speedupy,
...
ax0.set_yticks(np.arange(400)[::50])
ax0.set_yticklabels(np.arange(400)[::50] * 726 / 1.0e3)
ax0.set_ylabel("Radial Distance (Mm)")
ax1.plot(225 + np.arange(len(Tbsubmax[0])), Tbsubmax[0] / 1.0e6, "o-")
ax1.set_yscale("log")
ax2.plot(225 + np.arange(len(ycsub90[0])), ycsub90[0], "o-")
ax2.set_ylim([-600, 0])
ax3.plot(225 + np.arange(len(ycsub90[0])), pa_angle, "o")
ax3.set_xlabel("Time (HH:MM:SS) UT")
ax3.set_ylabel("P.A. (degrees)")
ax2.set_ylabel("Y-Coordinate (arcsec)")
...
ax.plot(
    rhessi_spec[1].data["CONVFAC"][0] + rhessi_spec[1].data["PHMODEL"][0],
    "o-",
    label="CONVFAC+PHMODEL",
)
ax.set_xscale("log")
ax.set_yscale("log")
ax.legend()
