...
MOMMS='/media/rohit/VLA/20160409_selfcal/sun_L_20160409T1844-1846UT.50ms.cal.ms/' 
split_=1
if(split_==1):
    OUTPUT_MS='sun_L_20160409T184401.00-184401.05UT.50ms.cal.ms'
    os.system('rm -rf '+OUTPUT_MS)
    timerange_='18:45:00.00~18:45:00.05' 
    datacolumn_='data'
...
spw_final='0:55~60'
timerange_final='18:45:00.00~18:45:00.05'
antennas_=''#'ea01,ea03,ea06,ea07,ea08,ea09,ea17,ea18,ea19,ea21,ea23,ea25,ea26,ea27'
refant_=''#'ea07'
# RA:02 05 i40.74 DEC:+12 44 01.6 20:34:00
#solar_centre='J2000 01h14m04.7s  +7d47m42.5s' # For 203430-203431
#phase_centre='J2000 01h14m04.7s  +7d47m42.5s' # From Yingjie script
phase_centre='J2000 01h14m43.863s  +07d56m01.107s'
phase_centre_mod=phase_centre
masks_=[]
    ...
    for step in range(len(modes)):
        ...
        if(make_model):
            ...
            if(interactive==True):
                ...
                imsize=imsize_,cell=cell_,phasecenter=phase_centre_mod, \
        ...
        if(make_image):
            ...
            clean(vis=MSI,imagename=image_f[step], \
                    ...
                    stokes='LL',weighting="natural",robust=0.0,uvtaper=False,outertaper=[''],innertaper=['1.0'], \
...
clearcal('sun_L_20160409T1844-1846UT.50ms.cal.ms')
for i in range(8):
    #applycal(vis="sun_L_20160409T1844-1846UT.50ms.cal.ms",spw=str(i),gaintable=['selfsol/fsun.selfcal_soln.spw.'+str(i)+'.cal.2.a', 'selfsol/fsun.selfcal_soln.spw.'+str(i)+'.cal.5.p'],calwt=[False],applymode="calonly")
    applycal(vis="sun_L_20160409T1844-1846UT.50ms.cal.ms",spw=str(i),gaintable=['selfsol/fsun.selfcal_soln.spw.'+str(i)+'.cal.0.p','selfsol/fsun.selfcal_soln.spw.'+str(i)+'.cal.1.p','selfsol/fsun.selfcal_soln.spw.'+str(i)+'.cal.2.a', \
        'selfsol/fsun.selfcal_soln.spw.'+str(i)+'.cal.3.p','selfsol/fsun.selfcal_soln.spw.'+str(i)+'.cal.4.p','selfsol/fsun.selfcal_soln.spw.'+str(i)+'.cal.5.p'],calwt=[False],applymode="calonly")
split(vis='sun_L_20160409T1844-1846UT.50ms.cal.ms',outputvis='sun_L_20160409T1844-1846UT.50ms.selfcal_final.ms',datacolumn="corrected")
