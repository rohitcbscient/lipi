...
for step in range(len(modes)):
    ...
    if(apply_model):
        ...
        #delmod(vis=MS)
        ft(vis=MS,field="",spw=spw_,model=model_[step+1],nterms=1,reffreq="",complist="",incremental=True,usescratch=True)
    # gaincal
    cal_gaincal=1
    if(cal_gaincal):
    ...
    if(plot_cal_amp):
        plotcal(caltable=caltable_,xaxis="antenna",yaxis="amp",poln="L",
                ...
                iteration="",plotrange=[],showflags=False,plotsymbol="o",plotcolor="blue",markersize=5.0,
    ...
    img_std=imstat(imagename=image_f[step]+".image/",axes=-1,region="",box="10,10,100,100",chans="",stokes="",listit=True, \
            verbose=True,mask="",stretch=False,logfile="",append=True,algorithm="classic",fence=-1, \
            center="mean",lside=True,zscore=-1,maxiter=-1,clmethod="auto",niter=3)
