...
x, y, z, bx, by, bz = Bextrap.get_fieldlines(
    "/media/rohit/VLA/paraview/EUV_loop_radio.csv"
)
x, y, z, bx, by, bz, b_hp, b_hp_pix, b_carr, b_carr_pix = Bextrap.transform_fieldlines(
    x, y, z, bx, by, bz, "2016/04/09T18:45:00", bsmap[0].wcs, bcarrmap[0]
)
...
x0, y0, sigx0, sigy0, rot0, tb0, x1, y1, sigx1, sigy1, rot1, tb1 = np.load(
    "/media/rohit/VLA/20160409/blob/all_params.npz.npy"
)
...
index["WCSNAME"], index["CTYPE1"], index["CUNIT1"], index["CTYPE2"], index["CUNIT2"] = (
...
hp_coord = mymap.reference_coordinate.transform_to(
    frames.Helioprojective(observer="earth")
)
...
aiamap = Map(
    "/media/rohit/VLA/20160409_EUV/171/ssw_cutout_20160409_184434_AIA_171_.fts"
)
...
def get_fieldlines(file_name, out_wcs):
    ...
    dd1 = mymap.pixel_to_world(x[1:] * u.pix, y[1:] * u.pix)
    dd = dd1.transform_to("heliocentric")
    xkm = dd.cartesian.x.value
    ykm = dd.cartesian.y.value
    zkm = dd.cartesian.z.value
    sc = SkyCoord(
        xkm * u.km,
        ykm * u.km,
        zkm * u.km,
        obstime="2016/04/09T18:45:00",
        observer="earth",
        frame="heliocentric",
    )
...
tb0_pk1 = tb0[:, 840:910]
tb0_pk2 = tb0[:, 930:1000]
tb0_pk3 = tb0[:, 1040:1100]
tb0_pk4 = tb0[:, 1130:1200]
tb0_pk5 = tb0[:, 1270:1350]
tb0_pk6 = tb0[:, 1400:1460]
tb1_pk1 = tb1[:, 840:910]
tb1_pk2 = tb1[:, 930:1000]
tb1_pk3 = tb1[:, 1040:1100]
tb1_pk4 = tb1[:, 1130:1200]
tb1_pk5 = tb1[:, 1270:1350]
tb1_pk6 = tb1[:, 1400:1460]
y0_pk1 = y0[:, 840:910]
y0_pk2 = y0[:, 930:1000]
y0_pk3 = y0[:, 1040:1100]
y0_pk4 = y0[:, 1130:1200]
...
x1_pk6 = x1[:, 1400:1460]
listvla_r = sorted(
    glob.glob("/media/rohit/VLA/20160409/images_50ms_RR/spw_3/*spw.*0-15*.FITS")
)[0:2000]
mapp = Map(listvla_r[800])
dcp_data1 = mapp.data
dcp_data1[np.isnan(dcp_data1)] = 0
dcp_data1[np.where(dcp_data1 < 0)] = 0
listvla_r = sorted(
    glob.glob("/media/rohit/VLA/20160409/images_50ms_RR/spw_5/*spw.*0-15*.FITS")
)[0:2000]
mapp = Map(listvla_r[800])
dcp_data2 = mapp.data
dcp_data2[np.isnan(dcp_data2)] = 0
dcp_data2[np.where(dcp_data2 < 0)] = 0
listvla_r = sorted(
    glob.glob("/media/rohit/VLA/20160409/images_50ms_RR/spw_0/*spw.*0-15*.FITS")
)[0:2000]
mapp = Map(listvla_r[800])
dcp_data0 = mapp.data
dcp_data0[np.isnan(dcp_data0)] = 0
dcp_data0[np.where(dcp_data0 < 0)] = 0
dd0 = Map(dcp_data0, mapp.meta)
dd1 = Map(dcp_data1, mapp.meta)
dd2 = Map(dcp_data2, mapp.meta)
qmapp = Map(listvla_r[350])
qmapp_data = qmapp.data
qmapp_data[np.isnan(qmapp_data)] = 0
qmapp0 = Map(qmapp_data, qmapp.meta)
...
xlaia1 = aiamap.center.Tx.value - 2.0 * int(aiamap.data.shape[0] / 2)
xraia1 = aiamap.center.Tx.value + 2.0 * int(aiamap.data.shape[0] / 2)
ylaia1 = aiamap.center.Ty.value - 2.0 * int(aiamap.data.shape[1] / 2)
yraia1 = aiamap.center.Ty.value + 2.0 * int(aiamap.data.shape[0] / 2)
...
if plot_fields:
    ...
    ax0.set_xlim(-860, -680)
    ax0.set_ylim(180, 360)
    plt.show()
...
if plot_fields:
    ...
    sc = ax0.scatter(
        b_hp_fr.Tx.value[idx1],
        b_hp_fr.Ty.value[idx1],
        s=5,
        c=z[idx1],
        vmin=100,
        vmax=400,
...
if plot_fields:
    ...
    ax0.contour(
        qmapp0.data / np.nanmax(qmapp0.data),
    ...
    # plt.plot(x0[j],y0[j],'o')
    # plt.plot(x1[j],y1[j],'o')
    ax0.set_xlim(-1000, -600)
    ax0.set_ylim(0, 400)
    plt.show()

freq = np.round(np.linspace(0.994, 2.006, 32), 3)
plot_paper_timeseries = 1
Tball0 = np.hstack((qsmaxTbr, maxTbr[0]))
Tball0[2132:2170] = np.nan
Tball0[4531:4570] = np.nan
qsmaxTbr[2132:2170] = np.nan
qsmaxTbr[4531:4570] = np.nan
if plot_paper_timeseries:
    f, ax = plt.subplots(1, 1)
    ax.plot(Tball0 / 1.0e6, "o-", label=str(np.round(freq[0])) + " GHz", markersize=2.0)
    ax.axvline(x=0, color="k")
    ax.axvline(x=6799, color="k")
    ax.axvline(x=4800, color="k")
    ax.axvspan(-1, 4800, color="yellow", alpha=0.2)
    ax.axvspan(4800, 6800, color="gray", alpha=0.2)
    ax.set_xticks(np.arange(len(Tball0))[::900])
    ax.set_xticklabels(
        [
            "18:40:00",
            "18:40:45",
...
if plot_paper_timeseries:
    f, ax = plt.subplots(1, 1)
    ax.plot(np.array(qsmaxTbr) / 1.0e6, "o-", label=str(freq[0]) + " GHz")
    ax.set_xticks(np.arange(len(qsmaxTbr))[::1000])
    ax.set_xticklabels(["18:44:00", "18:44:50", "18:45:40", "18:46:30", "18:47:20"])
    ax.legend()
    ax.set_ylabel("$T_B$ (MK)")
    ax.set_xlabel("Time (HH:MM:SS UT)")
    plt.show()
if plot_paper_timeseries:
    f, ax = plt.subplots(1, 1)
    ax.plot(np.array(maxTbr[0]) / 1.0e6, "o-", label=str(freq[0]) + " GHz")
    ax.set_xticks(np.arange(len(maxTbr[0]))[::500])
    ax.set_xticklabels(["18:48:00", "18:48:25", "18:48:50", "18:49:15", "18:49:45"])
    ax.legend()
    ax.set_ylabel("$T_B$ (MK)")
    ax.set_xlabel("Time (HH:MM:SS UT)")
    plt.show()
...
ro = d["ro"] * 1.0e17
temp = d["te"] * 1.0e4
fpro = 9000 * np.sqrt(ro) / 1.0e9
f, ax = plt.subplots(1, 1)
ax.plot(x, fpro[0], "o-", label="Plasma Frequency")
ax.plot(h, omegap, "o-", label="Gyro-resonance Freqeuncy")
ax.set_xlabel("Coronal Height (Mm)")
ax.set_ylabel("Freqeuncy (GHz)"), ax.legend()
plt.show()
...
plt.show()
plt.plot((r[81, 154] - 1) * 700, 9000 * np.sqrt(dens[81, 154]) / 1.0e6, "o-")
plt.xlabel("Coronal Height (Mm)")
plt.ylabel("Plasma Freqeuncy (MHz)")
plt.show()

f, ax = plt.subplots(1, 1)
ax.plot(x, ro[0], "o-", label="Density")
ax1 = ax.twinx()
ax1.plot(x, temp[0] / 1.0e6, "o-", label="Temperature", color="k")
...
R = np.linspace(2000, 280.0e3, 1000)
nkne = 4.2e4 * 10 ** (4.32 / ((695700.0 + R) / 695700.0))
th = 10 * np.pi / 180.0
saitone = (
    (3.09e8 * (1 - 0.5 * np.sin(th)) / ((695700.0 + R) / 695700.0) ** 16)
    + (1.58e8 * (1 - 0.95 * np.sin(th)) / ((695700.0 + R) / 695700.0) ** 6)
    + (0.0251e8 * (1 - np.sqrt(np.sin(th))) / ((695700.0 + R) / 695700.0) ** 2.5)
)
nkfp = 9000 * np.sqrt(nkne)
saitofp = 9000 * np.sqrt(saitone)

# plt.plot(R,nkne)
plt.plot(R, nkfp / 1.0e6, label="Newkirk")
plt.plot(R, saitofp / 1.0e6, label="SAITO")
plt.legend()
# plt.xlabel('Coronal Height (km)');plt.ylabel('Electron Density (ccm)')
plt.xlabel("Coronal Height (km)")
plt.ylabel("Plasma Frequency (MHz)")
plt.show()

ne25 = 1.0e9 + 1.16e17 * np.exp(-1 * (h * 1000 / 500.0))
fp25 = 9000 * np.sqrt(ne25) / 1.0e9
plt.plot(h, omegap, "o-", label="Gyroresonce Freqeuncy")
...
plt.imshow(
    ...
    aspect="auto",
    origin=0,
    extent=[xc - 0.6 * 83, xc + 83, yc - 0.6 * 83, yc + 0.6 * 83],
)
...
omegap_dem = np.array(omegap_dem)
# dem_arr[np.where((dem_arr<2.e27) & (dem_arr>1.e27))]=np.nan

s = 2
sf = np.math.factorial(s)
e = 1.0e-19
me = 9.1e-31
c = 3.0e8
nu = 1.0e9
...
def optical_depth(s, ratio):
    Ftho = (
        ...
        * np.sin(th * np.pi / 180)
        * (
            np.sin(th * np.pi / 180) ** 2
            + 4 * np.cos(th * np.pi / 180) ** 2
            + np.sqrt(
                np.sin(th * np.pi / 180) ** 4 + 16 * np.cos(th * np.pi / 180) ** 2
            )
        )
        ** 2
    ) / (
        np.sin(th * np.pi / 180) ** 4
        + 16 * np.cos(th * np.pi / 180) ** 2
        + np.sin(th * np.pi / 180)
        * np.sin(th * np.pi / 180)
        * np.sqrt(np.sin(th * np.pi / 180) ** 4 + 16 * np.cos(th * np.pi / 180) ** 2)
    )
    Fthx = (
        np.sin(th * np.pi / 180)
        * np.sin(th * np.pi / 180)
...
plt.imshow(
    np.log10(dem_arr), origin=0, aspect="auto", extent=[h[0], h[-1], sh[0], sh[-1]]
)
plt.colorbar(label="Log(EM(cm$^{-5}$))")
plt.ylabel("Scale Height (km)")
plt.xlabel("Source Height (Mm)")
plt.show()

plt.plot(sh, wpe_dem / omegap_dem, "o-")
plt.ylabel("$\omega_p/\Omega_e$")
...
ax[1, 1].plot(
    h, 9000 * np.sqrt(ne[28]) / 1.0e9, "-", label="Plasma Freqeuncy h0=150 km"
)
ax[1, 1].plot(h, omegap, "o-", label="Gyroresonce Frequency")
ax[1, 1].set_ylabel("Frequency (GHz)")
ax[1, 0].set_xlabel("Coronal Height (Mm)")
ax[1, 1].set_xlabel("Coronal Height (Mm)")
ax[0, 0].legend()
ax[0, 1].legend()
ax[1, 0].legend()
ax[1, 1].legend()
ax[0, 0].text(2, 20, "(I)", fontsize=15)
ax[1, 0].text(2, 0.02, "(III)", fontsize=15)
ax[0, 1].text(2, 1.0e9, "(II)", fontsize=15)
ax[1, 1].text(2, 0.02, "(IV)", fontsize=15)
plt.show()

f, ax = plt.subplots(1, 1)
ax.plot(h, 9000 * np.sqrt(ne[28]) / 1.0e9, "-", label="Plasma Freqeuncy h0=150 km")
...
ax1.set_xscale("log")
ax1.set_yscale("log")
ax.set_xlabel("Coronal Heights (Mm)")
ax.set_ylabel("Freqeuncy (GHz)")
ax1.set_ylabel("$\\tau$")
plt.show()

vA = babs[:, 160, 172] / np.sqrt(4 * np.pi * ne[28] * 9.1e-28) / 1.0e5
f, ax = plt.subplots(1, 1)
ax.plot(h, 9000 * np.sqrt(ne[28]) / 1.0e9, "-", label="Plasma Freqeuncy h0=150 km")
ax.plot(h, omegap, "o-", label="Gyroresonce Frequency")
ax1 = ax.twinx()
...
plt.plot(h, omegap, "o-", label="Gyroresonce Frequency")
plt.plot(h1, 9000 * np.sqrt(ne[28]) / 1.0e9, "-", label="Plasma Frequency h0=150 km")
# plt.axhline(y=1.4,color='k');plt.axhline(y=0.99,color='k')
# plt.axvline(x=20.0,color='k');plt.axvline(x=26.0,color='k')
plt.axvline(x=hh2_0[0], color="k")
plt.axvline(x=hh2_0[16], color="k")
plt.axvline(x=hh2_0[30], color="k")
plt.xlabel("Coronal Height (Mm)")
plt.ylabel("Frequency (GHz)"), plt.legend()
plt.yscale("log")
plt.xscale("log")
...
ax.legend()
ax.set_ylim(1.0e8, 1.0e9)
ax.set_xscale("log")
ax.set_xlim(1.0e1, 1.0e2)
ax.set_ylabel("Density ($cm^{-3}$)")
ax.set_xlabel("Coronal Height (Mm)")
plt.show()
...
echarge_cgs = 4.4e-10
lnL = 20
K = 2 * np.pi * echarge_cgs**4 * lnL  # units are (erg cm)^2
E = np.array([1, 2, 3, 4, 5, 8, 10, 15, 20, 25, 30])  # in keV
ve_stop = 300 * np.sqrt(1 - (1 / (0.002 * E + 1) ** 2))
eV2ergs = 1.6e-12
ne_stop_top = 3.0e7
Lstop_top = (E * 1.0e3 * eV2ergs) ** 2 / (2 * K * ne_stop_top) / 1.0e8  # in Mm
tstop_top = Lstop_top / ve_stop
tau_col_top = (5210 * (2.0e6) ** 2 / ne_stop_top) / 1.0e8 / ve_stop
Dm_top = 0.7
Ldrift_top = np.sqrt(2 * Dm_top * Lstop_top)
# ----
ne_stop_1p5 = 6.0e8
Lstop_1p5 = (E * 1.0e3 * eV2ergs) ** 2 / (2 * K * ne_stop_1p5) / 1.0e8  # in Mm
tstop_1p5 = Lstop_1p5 / ve_stop
tau_col_1p5 = (5210 * (2.0e6) ** 2 / ne_stop_1p5) / 1.0e8 / ve_stop
...
for i in range(len(E)):
    print(
        ...
        " Mm/s",
        "&",
        np.round(tau_col_top[i], 2),
        " / ",
        np.round(tau_col_1p5[i], 3),
        "&",
        np.round(Lstop_top[i], 1),
        " / ",
        np.round(Lstop_1p5[i], 1),
        "&",
        np.round(tstop_top[i], 1),
        " / ",
        np.round(tstop_1p5[i], 1),
        "&",
        np.round(Ldrift_top[i], 1),
        "\\\\",
    )
