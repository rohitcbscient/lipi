...
z_1000 = z[idx1000].max() * 1.4
z_1500 = z[idx1500].max() * 1.4
z_1200 = z[idx1200].max() * 1.4

####
penergy = np.linspace(1, 20, 100)
# fermi_energy=1 # keV
pitch_angle = np.logspace(0, np.log10(90), 10000000)
omegapbygyro = 0.3
R = z_1000 - z_1500
B_low = 200
B_top = 40
...
lambda_ei = 5.21e3 * T * T / ne_1p5GHz
lambda_turb = lambda_ei * np.e
tcc = [0] * len(penergy)
bounce_time = [0] * len(penergy)
w = [0] * len(penergy)
t_loss = [0] * len(penergy)
slength = [0] * len(penergy)
slength_turb = [0] * len(penergy)
i = 0
for fermi_energy in penergy:
    ...
    bounce_time[i] = (
        2 * np.pi * B_scale_height / (v[i] * np.sin(pitch_angle * np.pi / 180))
    )
    tcc[i] = bounce_time[i] / np.pi * np.arccos((s_cl1000) / (mirror_point))
    epsilonD = 1 / ((np.pi * loop_length * 1000) / (2 * 726 * 18))  # 18" radius
    Tt = 2.0e6
    lambda_ei = (
        5210 * Tt**2 / (ne_1p5GHz) / 1.0e8
    )  # (CGS) in Mm ne[28][72] is ne for the 1.5 GHz
    w[i] = tcc[i] / (lambda_ei / v[i])
    t_loss[i] = (lambda_ei / v[i]) * (
        0.5 * np.pi / np.arccos((s_cl1000) / (mirror_point))
    )
    slength[i] = (
        10
        * (fermi_energy**2 / 400)
        * (10**11 / ne_1d_low)
        * (np.cos(pitch_angle * np.pi / 180))
    )
    slength_turb[i] = np.sqrt(lambda_turb * 1.0e8 / 9.3e-36 / ne_1d_low) * penergy
    i = i + 1
tcc = np.array(tcc)
bounce_time = np.array(bounce_time)
v = np.array(v)
w = np.array(w)
slength = np.array(slength)
slength[slength > 60] = np.nan
slength[slength < 20] = np.nan
slength_p = slength * 1.0
t_loss = np.array(t_loss)
slength_p[0:9] = np.nan
slength_p[19:] = np.nan
...
slength_diff = np.sqrt(lambda_ei / K / ne_1d_low) * penergy / 1.0e8
...
ax.set_xlim([500, 980])
tickxidx = [200, 500, 600, 700, 800, 980]
ax.set_xticks([])
...
f, ax = plt.subplots(1, 1)
im = ax.imshow(
    tcc, aspect="auto", origin="lower", cmap="jet", norm=LogNorm(vmin=0.5, vmax=3)
)
ax.set_xscale("linear")
ax.set_xlim([500, 980])
tickxidx = [200, 500, 600, 850, 900, 950, 980]
ax.set_xticks([])
ax.set_xticks(tickxidx)
ax.set_xticklabels(np.round(pitch_angle[tickxidx], 1))
ax.set_yticks(np.arange(len(penergy))[::10])
...
f, ax = plt.subplots(1, 1)
im = ax.imshow(
    t_loss, aspect="auto", origin="lower", cmap="jet", norm=LogNorm(vmin=0.01, vmax=0.5)
)
ax.set_xscale("linear")
ax.set_xlim([500, 980])
tickxidx = [200, 300, 400, 450, 500, 500, 600, 650, 700, 750, 800]
ax.set_xticks([])
ax.set_xticks(tickxidx)
ax.set_xticklabels(np.round(pitch_angle[tickxidx], 1))
ax.set_yticks(np.arange(len(penergy))[::10])
...
ax.set_xscale("linear")
ax.set_xlim([800, 990])
tickxidx = [200, 500, 600, 700, 750, 850, 900, 950, 980]
ax.set_xticks([])
ax.set_xticks(tickxidx)
ax.set_xticklabels(np.round(pitch_angle[tickxidx], 1))
ax.set_yticks(np.arange(len(penergy))[::10])
...
alp0 = 0.4
masing_length = 6.0e9
Gamma = 2 * np.pi * 0.005 * 1.0e9
facc = v0 / (L * Gamma)
W = facc * n0 * m0 * v0 * v0 * alp0**3
A = np.pi * (18 * 726) ** 2 * 1.0e10
V = A * masing_length
rad_energy = S * 1.0e-19 * A * 1.0e9 * 2.5
a = 8
eta1 = (
    0.5
    * (a - 3)
...
print(
    "Number of masing cells:",
    A / (rad_energy / W / masing_length),
    "Gamma:",
    Gamma / 1.0e5,
)
print("Ratio:", W * V / rad_energy)
del_alph = (rad_energy / V / facc / n0 / m0 / v0 / v0) ** (1.0 / 3)
...
I_ratio = np.sqrt(
    tau_growth / tau_diff_eff
)  # Ampltitude of perturbations in the number distribution to density

tof0 = 100  # sec
alpha_trap0 = np.rad2deg(np.arcsin(2.0e9 / tof0 / v0))
...
plt.ylabel("Time (sec)")
plt.xlabel("$\\alpha_0$ (degrees)")
plt.legend()  # ;plt.ylim(0.01,1)
plt.yscale("log")
plt.show()
...
plt.plot(
    np.linspace(20, 55, 10), np.ones(10) * 35, "--", color="k", label="Mirroring layers"
)
...
plt.plot(
    ...
    linewidth=3,
    color="k",
    label="ECM Loop top Width (35 Mm) / L$_{drift}$",
)
plt.plot(
    np.ones(10) * 0,
    np.linspace(50, 140, 10),
    ":",
    linewidth=3,
    color="k",
    label="Loop top Width (90 Mm) / $w_{top}$",
)
plt.legend()
plt.plot(
    np.linspace(-54, 75, 10),
...
if plot_fields:
    ...
    ax0 = fig.add_subplot(121, projection=bsmap[0])
    ax1 = fig.add_subplot(122, projection=bcarrmap[0])
    ...
    seeds1 = SkyCoord(
        b_carr.lon.ravel(), b_carr.lat.ravel(), frame=bcarrmap[0].coordinate_frame
    )
...
Mm2cm = 1.0e8
volume = np.pi * (18 / 2.0) ** 2 * 60
stopping_depth = 0.92e17 * penergy**2
ne = 6.0e8
stopping_length = 2.0e17 * penergy**2 / ne / 1.0e8
...
for i in range(len(ve)):
    for j in range(len(pitch_angle_array)):
        travel_time[i][j] = 115.0 / (ve[i] * np.cos(pitch_angle_array[j] * np.pi / 180))

energy_stopping_length = np.sqrt(60 / 2.0e17 * ne * 1.0e8)  # stopping width = 60 Mm
energy_stopping_idx = ut.find_nearest(penergy, energy_stopping_length)[0]

ne_low = 1.0e8
energy_stopping_length_low = np.sqrt(
    115 / 2.0e17 * ne_low * 1.0e8
)  # stopping width = 60 Mm
energy_stopping_idx_low = ut.find_nearest(penergy, energy_stopping_length_low)[0]
travel_time1 = 1.0 * travel_time
travel_time1[travel_time1 < 40] = np.nan
travel_time1[travel_time1 > 50] = np.nan
travel_time1[0:energy_stopping_idx_low] = np.nan
travel_time1[energy_stopping_idx:] = np.nan
...
ax.set_xticks(np.arange(len(pitch_angle_array))[::10])
ax.set_xticklabels(np.round(pitch_angle_array, 1)[::10])
ax.set_yticks(np.arange(len(penergy))[::10])
ax.set_yticklabels(np.round(penergy, 1)[::10])
