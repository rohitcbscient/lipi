...
xc_max1=np.array(xc_max)
yc_max1=np.array(yc_max)
...
ax1.set_title('DS')
im1=ax1.imshow(max_/1.e6,extent=[0,5,freq[0],freq[-1]],aspect='auto',interpolation=None,cmap='jet',origin='lower')
divider = make_axes_locatable(ax1)
cax = divider.append_axes('right', size='5%', pad=0.05)
fig.colorbar(im1,cax=cax,orientation='vertical',label='MK')
ax2 = fig.add_subplot(spec[1, 0],sharex=ax1,sharey=ax1)
ax2.set_title('XC')
im2=ax2.imshow(xc_max1,extent=[0,5,freq[0],freq[-1]],aspect='auto',interpolation=None,cmap='jet',origin='lower',vmin=620,vmax=690)
divider = make_axes_locatable(ax2)
cax = divider.append_axes('right', size='5%', pad=0.05)
fig.colorbar(im2,cax=cax,orientation='vertical',label='arcsec')
ax3 = fig.add_subplot(spec[1, 1],sharex=ax2,sharey=ax2)
ax3.set_title('YC')
im3=ax3.imshow(yc_max1,extent=[0,5,freq[0],freq[-1]],aspect='auto',interpolation=None,cmap='jet',origin='lower',vmin=240,vmax=260)
