...
def idl2sunpy_hmi(mapsav):
    ...
    header[
        "RSUN_OBS"
    ] = 958.11  # sunpy.coordinates.sun.angular_radius(header['DATE-OBS']).value
    header["DSUN_OBS"] = (
        sunpy.coordinates.sun.earth_distance(header["DATE-OBS"]).to(u.meter).value
    )
...
def get_sunpy_rundiff_maps(f, wave, inst):
    ...
    for i in range(1, n):
        if inst == "AIA":
            ab = idl2sunpy_sdo(f[i - 1], wave, inst)
            hb, db = ab.meta, ab.data
            aa = idl2sunpy_sdo(f[i], wave, inst)
            h, d = aa.meta, aa.data
        if inst == "HMI":
            ab = idl2sunpy_hmi(f[i - 1])
            hb, db = ab.meta, ab.data
            aa = idl2sunpy_hmi(f[i])
            h, d = aa.meta, aa.data
        if d.shape[0] == db.shape[0]:
...
def change_pointing(f, xc, yc):
    g = fits.open(f)
    g[0].header["CRVAL1"] = xc
    g[0].header["CRVAL2"] = yc
    g.writeto(
        "/media/rohit/VLA/20160409/sun_L_20160409T1844-1846UT.50ms.cal.pol.LL.spw.0_16-31.time.18:45:10.-18:45:10.05.fits"
    )
    g.close()
...
if dump_submaps:
    ...
    allaiamaps = {}
    allaiamaps["aia94"] = {"map94": map94, "data94": data94, "time94": time94}
    ...
    print("Writing..")
    pickle.dump(
        allaiamaps, open("/media/rohit/VLA/20160409/20160409_submap_aia_50ms.p", "wb")
    )
    ...
    allmapsd = {}
    allmapsd["aiad94"] = {"mapd94": mapd94, "datad94": datad94, "timed94": timed94}
    allmapsd["aiad131"] = {
        "mapd131": mapd131,
        "datad131": datad131,
        "timed131": timed131,
    ...
    pickle.dump(
        allmapsd,
        open("/media/rohit/VLA/20160409/20160409_submap_aia_diff_50ms.p", "wb"),
    )
    ##########################
    ...
    allmapsb = {}
    allmapsb["aiab94"] = {"mapb94": mapb94, "datab94": datab94, "timeb94": timeb94}
    allmapsb["aiab131"] = {
        "mapb131": mapb131,
        "datab131": datab131,
        "timeb131": timeb131,
    ...
    pickle.dump(
        allmapsb,
        open("/media/rohit/VLA/20160409/20160409_submap_aia_base_50ms.p", "wb"),
    )
    ###########################
...
if dump_submaps:
    ...
    listvla0 = sorted(
        glob.glob("/media/rohit/VLA/20160409/images_50ms_RR/spw_0/*.spw.0_*FITS")
    )
    listvla1 = sorted(
        glob.glob("/media/rohit/VLA/20160409/images_50ms_RR/spw_1/*.spw.1_*FITS")
    )
    listvla2 = sorted(
        glob.glob("/media/rohit/VLA/20160409/images_50ms_RR/spw_2/*.spw.2_*FITS")
    )
    listvla3 = sorted(
        glob.glob("/media/rohit/VLA/20160409/images_50ms_RR/spw_3/*.spw.3_*FITS")
    )
    listvla4 = sorted(
        glob.glob("/media/rohit/VLA/20160409/images_50ms_RR/spw_4/*.spw.4_*FITS")
    )
    listvla5 = sorted(
        glob.glob("/media/rohit/VLA/20160409/images_50ms_RR/spw_5/*.spw.5_*FITS")
    )
    listvla6 = sorted(
        glob.glob("/media/rohit/VLA/20160409/images_50ms_RR/spw_6/*.spw.6_*FITS")
    )
    listvla7 = sorted(
        glob.glob("/media/rohit/VLA/20160409/images_50ms_RR/spw_7/*.spw.7_*FITS")
    )
    mapvla = [0] * 2399
    datavla = [0] * 2399
    mapvla0 = [0] * 2399
    ...
    for i in range(0, 2399):
        ...
        if (
            os.path.isfile(
                "/media/rohit/VLA/20160409/vlamaps_RR/vlamap_"
                + str(listvla0[i].split(".")[8])
                + "_"
                + "%04d" % i
                + ".p"
            )
            ...
            datavla7[i] = [0] * 4
            for k in range(4):
                mapvla0[i][k], datavla0[i][k], timevla = get_evla_submap(
                    [listvla0[i + 2399 * k]], 0, -1, 0, -1
                )
                mapvla1[i][k], datavla1[i][k], timevla = get_evla_submap(
                    [listvla1[i + 2399 * k]], 0, -1, 0, -1
                )
                mapvla2[i][k], datavla2[i][k], timevla = get_evla_submap(
                    [listvla2[i + 2399 * k]], 0, -1, 0, -1
                )
                mapvla3[i][k], datavla3[i][k], timevla = get_evla_submap(
                    [listvla3[i + 2399 * k]], 0, -1, 0, -1
                )
                mapvla4[i][k], datavla4[i][k], timevla = get_evla_submap(
                    [listvla4[i + 2399 * k]], 0, -1, 0, -1
                )
                mapvla5[i][k], datavla5[i][k], timevla = get_evla_submap(
                    [listvla5[i + 2399 * k]], 0, -1, 0, -1
                )
                mapvla6[i][k], datavla6[i][k], timevla = get_evla_submap(
                    [listvla6[i + 2399 * k]], 0, -1, 0, -1
                )
                mapvla7[i][k], datavla7[i][k], timevla = get_evla_submap(
                    [listvla7[i + 2399 * k]], 0, -1, 0, -1
                )
            mapvla[i] = (
                mapvla0[i]
                + mapvla1[i]
            ...
            allmaps = {}
            allmaps["vla"] = {"mapvla": mapvla[i], "timevla": timevla}
            pickle.dump(
                allmaps,
                open(
                    "/media/rohit/VLA/20160409/vlamaps_RR/vlamap_"
    ...
    pickle.dump(
        allvlamaps0, open("/media/rohit/VLA/20160409/20160409_vla_spw_0_50ms.p", "wb")
    )
    allvlamaps1["vla1"] = {"mapvla": mapvla1, "datavla": datavla1, "timevla": timevla1}
    pickle.dump(
        allvlamaps1, open("/media/rohit/VLA/20160409/20160409_vla_spw_1_50ms.p", "wb")
    )
    allvlamaps2["vla2"] = {"mapvla": mapvla2, "datavla": datavla2, "timevla": timevla2}
    pickle.dump(
        allvlamaps2, open("/media/rohit/VLA/20160409/20160409_vla_spw_2_50ms.p", "wb")
    )
    allvlamaps3["vla3"] = {"mapvla": mapvla3, "datavla": datavla3, "timevla": timevla3}
    pickle.dump(
        allvlamaps3, open("/media/rohit/VLA/20160409/20160409_vla_spw_3_50ms.p", "wb")
    )
    allvlamaps4["vla4"] = {"mapvla": mapvla4, "datavla": datavla4, "timevla": timevla4}
    pickle.dump(
        allvlamaps4, open("/media/rohit/VLA/20160409/20160409_vla_spw_4_50ms.p", "wb")
    )
    allvlamaps5["vla5"] = {"mapvla": mapvla5, "datavla": datavla5, "timevla": timevla5}
    pickle.dump(
        allvlamaps5, open("/media/rohit/VLA/20160409/20160409_vla_spw_5_50ms.p", "wb")
    )
    allvlamaps6["vla6"] = {"mapvla": mapvla6, "datavla": datavla6, "timevla": timevla6}
    pickle.dump(
        allvlamaps6, open("/media/rohit/VLA/20160409/20160409_vla_spw_6_50ms.p", "wb")
    )
    allvlamaps7["vla7"] = {"mapvla": mapvla7, "datavla": datavla7, "timevla": timevla7}
    pickle.dump(
        allvlamaps7, open("/media/rohit/VLA/20160409/20160409_vla_spw_7_50ms.p", "wb")
    )


def get_mapI(ml, mr):
    meta = ml.meta
    nd = (ml.data + mr.data) * 0.5
    mi = Map(nd, meta)
    return mi
...
if get_submap_pol:
    ...
    datavla_i = [0] * 32
    for k in range(32):
        mapvla_v[k], datavla_v[k], timevla = get_evla_submap(
            [listvla_v[k]], 0, -1, 0, -1
        )
        mapvla_i[k], datavla_i[k], timevla = get_evla_submap(
            [listvla_i[k]], 0, -1, 0, -1
        )
...
if get_max:
    ...
    for i in range(8):
        ...
        for j in range(4):
            listvla_l = sorted(
                glob.glob(
                    "/media/rohit/VLA/20160409/images_50ms_LL/spw_"
                    + str(i)
                    + "/*spw.*"
                    + spc[j]
                    + "*.FITS"
            ...
            listvla_r = sorted(
                glob.glob(
                    "/media/rohit/VLA/20160409/images_50ms_RR/spw_"
                    + str(i)
                    + "/*spw.*"
                    + spc[j]
                    + "*.FITS"
            ...
            listvla_v = sorted(
                glob.glob(
                    "/media/rohit/VLA/20160409/images_50ms_V/spw_"
                    + str(i)
                    + "/*spw.*"
                    + spc[j]
                    + "*.FITS"
            ...
            for k in range(2000):
                ...
                hl = ml[0]
                hr = mr[0]
                Tbi = (hl.data + hr.data) * 0.5
                maxTbi[i][j][k] = np.nanmax(Tbi)
                yc, xc = np.where(Tbi == np.nanmax(Tbi))
                xcimax[i][j][k] = (
                    hl.reference_coordinate.Tx.value
                    + (xc - (hl.reference_pixel.x.value - 1)) * hl.scale.axis1.value
                )
                ycimax[i][j][k] = (
                    hl.reference_coordinate.Ty.value
                    + (yc - (hl.reference_pixel.y.value - 1)) * hl.scale.axis2.value
                )
                Tbi_r1[i][j][k] = Tbi[138:148, 120:130].mean()
                Tbi_r2[i][j][k] = Tbi[124:136, 116:126].mean()
                eTbi[i][j][k] = Tbi[100:200, 15:85].std()
                areai50[i][j][k] = len(np.where(Tbi > np.nanmax(Tbi) * 0.5)[0]) * 2.0
                Tbi_f = Tbi * 1.0
                Tbi_f[np.isnan(Tbi_f)] = 0
                bi = ut.get_bimage(Tbi_f, 0.95)
                xcf, ycf = ut.fitEllipse(bi)[0:2]
                xci90[i][j][k] = (
                    hl.reference_coordinate.Tx.value
                    + (xcf - (hl.reference_pixel.x.value - 1)) * hl.scale.axis1.value
                )
                yci90[i][j][k] = (
                    hl.reference_coordinate.Ty.value
                    + (ycf - (hl.reference_pixel.y.value - 1)) * hl.scale.axis2.value
                )
                ######
                mv, dv, tv = get_evla_submap([listvla_v[k]], 0, -1, 0, -1)
                h = mv[0]
                Tbv = h.data
                maxTbv[i][j][k] = np.nanmax(Tbv)
                yc, xc = np.where(Tbv == np.nanmax(Tbv))
                xcvmax[i][j][k] = (
                    h.reference_coordinate.Tx.value
                    + (xc - (h.reference_pixel.x.value - 1)) * h.scale.axis1.value
                )
                ycvmax[i][j][k] = (
                    h.reference_coordinate.Ty.value
                    + (yc - (h.reference_pixel.y.value - 1)) * h.scale.axis2.value
                )
                Tbv_r1[i][j][k] = Tbv[138:148, 120:130].mean()
                Tbv_r2[i][j][k] = Tbv[124:136, 116:126].mean()
                eTbv[i][j][k] = Tbv[100:200, 15:85].std()
                Tbv_f = Tbv * 1.0
                Tbv_f[np.isnan(Tbv_f)] = 0
                bi = ut.get_bimage(Tbv_f, 0.95)
                xcf, ycf = ut.fitEllipse(bi)[0:2]
                xcv90[i][j][k] = (
                    h.reference_coordinate.Tx.value
                    + (xcf - (h.reference_pixel.x.value - 1)) * h.scale.axis1.value
                )
                ycv90[i][j][k] = (
                    h.reference_coordinate.Ty.value
                    + (ycf - (h.reference_pixel.y.value - 1)) * h.scale.axis2.value
                )
                #######
                h = ml[0]
                Tbl = h.data
                maxTbl[i][j][k] = np.nanmax(Tbl)
                ...
                Tbl_r1[i][j][k] = Tbl[138:148, 120:130].mean()
                Tbl_r2[i][j][k] = Tbl[124:136, 116:126].mean()
                eTbl[i][j][k] = Tbl[100:200, 15:85].std()
                Tbl_f = Tbl * 1.0
                Tbl_f[np.isnan(Tbl_f)] = 0
                ...
                Tbr_r1[i][j][k] = Tbr[138:148, 120:130].mean()
                Tbr_r2[i][j][k] = Tbr[124:136, 116:126].mean()
                eTbr[i][j][k] = Tbr[100:200, 15:85].std()
                Tbr_f = Tbr * 1.0
                Tbr_f[np.isnan(Tbr_f)] = 0
    ...
    areai50 = np.array(areai50)
    areai50 = areai50.reshape(32, 2000)
    maxTbi = np.array(maxTbi)
    maxTbi = maxTbi.reshape(32, 2000)
    eTbi = np.array(eTbi)
    eTbi = eTbi.reshape(32, 2000)
    eTbv = np.array(eTbv)
    eTbv = eTbv.reshape(32, 2000)
    eTbl = np.array(eTbl)
    eTbl = eTbl.reshape(32, 2000)
    eTbr = np.array(eTbr)
    eTbr = eTbr.reshape(32, 2000)
    xcimax = np.array(xcimax)
    xcimax = xcimax.reshape(32, 2000)
    ycimax = np.array(ycimax)
    ycimax = ycimax.reshape(32, 2000)
    xci90 = np.array(xci90)
    xci90 = xci90.reshape(32, 2000)
    yci90 = np.array(yci90)
    yci90 = yci90.reshape(32, 2000)
    xcv90 = np.array(xcv90)
    xcv90 = xcv90.reshape(32, 2000)
    ycv90 = np.array(ycv90)
    ycv90 = ycv90.reshape(32, 2000)
    xcl90 = np.array(xcl90)
    xcl90 = xcl90.reshape(32, 2000)
    ycl90 = np.array(ycl90)
    ycl90 = ycl90.reshape(32, 2000)
    xcr90 = np.array(xcr90)
    xcr90 = xcr90.reshape(32, 2000)
    ycr90 = np.array(ycr90)
    ycr90 = ycr90.reshape(32, 2000)
    Tbi_r1 = np.array(Tbi_r1)
    Tbi_r1 = Tbi_r1.reshape(32, 2000)
    Tbi_r2 = np.array(Tbi_r2)
    Tbi_r2 = Tbi_r2.reshape(32, 2000)
    pickle.dump(
        [xcimax, ycimax, xci90, yci90, maxTbi, Tbi_r1, Tbi_r2, areai50, eTbi],
        open("/media/rohit/VLA/20160409/vlamax_loc_i.p", "wb"),
    )
    maxTbv = np.array(maxTbv)
    maxTbv = maxTbv.reshape(32, 2000)
    xcvmax = np.array(xcvmax)
    xcvmax = xcvmax.reshape(32, 2000)
    ycvmax = np.array(ycvmax)
    ycvmax = ycvmax.reshape(32, 2000)
    Tbv_r1 = np.array(Tbv_r1)
    Tbv_r1 = Tbv_r1.reshape(32, 2000)
    Tbv_r2 = np.array(Tbv_r2)
    Tbv_r2 = Tbv_r2.reshape(32, 2000)
    pickle.dump(
        [xcvmax, ycvmax, xcv90, ycv90, maxTbv, Tbv_r1, Tbv_r2, eTbv],
        open("/media/rohit/VLA/20160409/vlamax_loc_v.p", "wb"),
    )
    maxTbl = np.array(maxTbl)
    maxTbl = maxTbl.reshape(32, 2000)
    xclmax = np.array(xclmax)
    xclmax = xclmax.reshape(32, 2000)
    yclmax = np.array(yclmax)
    yclmax = yclmax.reshape(32, 2000)
    Tbl_r1 = np.array(Tbl_r1)
    Tbl_r1 = Tbl_r1.reshape(32, 2000)
    Tbl_r2 = np.array(Tbl_r2)
    Tbl_r2 = Tbl_r2.reshape(32, 2000)
    pickle.dump(
        [xclmax, yclmax, xcl90, ycl90, maxTbl, Tbl_r1, Tbl_r2, eTbl],
        open("/media/rohit/VLA/20160409/vlamax_loc_l.p", "wb"),
    )
    maxTbr = np.array(maxTbr)
    maxTbr = maxTbr.reshape(32, 2000)
    xcrmax = np.array(xcrmax)
    xcrmax = xcrmax.reshape(32, 2000)
    ycrmax = np.array(ycrmax)
    ycrmax = ycrmax.reshape(32, 2000)
    Tbr_r1 = np.array(Tbr_r1)
    Tbr_r1 = Tbr_r1.reshape(32, 2000)
    Tbr_r2 = np.array(Tbr_r2)
    Tbr_r2 = Tbr_r2.reshape(32, 2000)
    pickle.dump(
        [xcrmax, ycrmax, xcr90, ycr90, maxTbr, Tbr_r1, Tbr_r2, eTbr],
        open("/media/rohit/VLA/20160409/vlamax_loc_r.p", "wb"),
    )

print("Reading VLA data.....")
freq = np.round(np.linspace(0.994, 2.006, 32), 3)

xcvmax, ycvmax, xcv90, ycv90, maxTbv, Tbv_r1, Tbv_r2, eTbv = pickle.load(
    open("/media/rohit/VLA/20160409/vlamax_loc_v.p", "rb"), encoding="latin1"
)
# Tbmayn[np.where(ycmax<255)]=np.nan;Tbmays[np.where(ycmax>255)]=np.nan
xcimax, ycimax, xci90, yci90, maxTbi, Tbi_r1, Tbi_r2, areai50, eTbi = pickle.load(
    open("/media/rohit/VLA/20160409/vlamax_loc_i.p", "rb"), encoding="latin1"
)
xclmax, yclmax, xcl90, ycl90, maxTbl, Tbl_r1, Tbl_r2, eTbl = pickle.load(
    open("/media/rohit/VLA/20160409/vlamax_loc_l.p", "rb"), encoding="latin1"
)
xcrmax, ycrmax, xcr90, ycr90, maxTbr, Tbr_r1, Tbr_r2, eTbr = pickle.load(
    open("/media/rohit/VLA/20160409/vlamax_loc_r.p", "rb"), encoding="latin1"
)
Tbmayn = maxTbr * 1.0
Tbmays = maxTbr * 1.0
...
if plot_polarisation:
    ...
    plt.errorbar(
        freq,
        maxTbv[:, qtime] / maxTbi[:, qtime] * 100,
        yerr=eTbv[:, qtime] / maxTbv[:, qtime] * 100,
        color="blue",
        label="Continuum (18:44:07.5-18:44:07.55 UT)",
    )
    plt.plot(freq, maxTbv[:, 858] / maxTbi[:, 858] * 100, "o", color="red")
    plt.errorbar(
        freq,
        maxTbv[:, 858] / maxTbi[:, 858] * 100,
        yerr=eTbv[:, 858] / maxTbv[:, 858] * 100,
        color="red",
        label="Bursts (18:44:42.9-18:44:42.95 UT)",
    )
    plt.legend()
    plt.xlabel("Frequency (GHz)")
    plt.ylabel("Degree of Polarisation (V/I %)")
    plt.ylim(0, 100)
    plt.show()

xx = np.log10(freq * 1.0e9)
ycn = np.log10(maxTbr[:, qtime])
ybs = np.log10(maxTbr[:, 858])
zc = np.polyfit(xx, ycn, 1, cov=True)
zb = np.polyfit(xx, ybs, 1, cov=True)
pc = np.poly1d(zc[0])
pb = np.poly1d(zb[0])
ycn_fit = 10 ** pc(xx)
ybs_fit = 10 ** pb(xx)
ezc = np.sqrt(zc[1][0][0])
ezb = np.sqrt(zb[1][0][0])
plot_Tb = 1
if plot_polarisation:
...
if get_submap_pol:
    listvla_v = sorted(
        glob.glob("/media/rohit/VLA/20160409/images_50ms_V/spw_0/*spw.0_16*.FITS")
    )[0:2000]
    listvla_ll = sorted(
        glob.glob("/media/rohit/VLA/20160409/images_50ms_LL/spw_0/*FITS")
    )[0:2000]
    listvla_rr = sorted(
        glob.glob("/media/rohit/VLA/20160409/images_50ms_RR/spw_0/*FITS")
    )[0:2000]
    mapvla_v = [0] * 2000
    datavla_v = [0] * 2000
    mapvla_i = [0] * 2000
    ...
    mapvla_r = [0] * 2000
    for k in range(2000):
        mapvla_v[k], datavla_v[k], timevla[k] = get_evla_submap(
            [listvla_v[k]], 0, -1, 0, -1
        )
    ...
    pickle.dump(
        [vlamax_v, datavla_v], open("/media/rohit/VLA/20160409/vlamax_v_spw0.p", "wb")
    )
vlamax_v, datavla_v = pickle.load(
    open("/media/rohit/VLA/20160409/vlamax_v_spw0.p", "rb")
)
...
if get_qs:
    ...
    Tbr_r1 = [0] * len(listqs)
    Tbr_r2 = [0] * len(listqs)
    arear50 = [0] * len(listqs)
    qstimevla = [0] * len(listqs)
    qsxcr90 = [0] * len(listqs)
    qsycr90 = [0] * len(listqs)
    for k in range(len(listqs)):
        m, d, qstimevla[k] = get_evla_submap([listqs[k]], 0, -1, 0, -1)
        hl = m[0]
        Tbr = hl.data
        maxTbr[k] = np.nanmax(Tbr)
        yc, xc = np.where(Tbr == np.nanmax(Tbr))
        xcrmax[k] = (
            hl.reference_coordinate.Tx.value
            + (xc - (hl.reference_pixel.x.value - 1)) * hl.scale.axis1.value
        ...
        Tbr_r1[k] = Tbr[138:148, 120:130].mean()
        Tbr_r2[k] = Tbr[124:136, 116:126].mean()
    ...
    pickle.dump(
        [xcrmax, ycrmax, qsxcr90, qsycr90, maxTbr, Tbr_r1, Tbr_r2, arear50, qstimevla],
        open("/media/rohit/VLA/20160409/vlamax_loc_r_qs_5.p", "wb"),
    )
(
    qsx,
    qsy,
    ...
    open("/media/rohit/VLA/20160409/vlamax_loc_r_qs_3.p", "rb"), encoding="latin1"
)
(
    qsx,
    qsy,
    ...
    open("/media/rohit/VLA/20160409/vlamax_loc_r_qs_5.p", "rb"), encoding="latin1"
)
(
    qsx,
    qsy,
    ...
    open("/media/rohit/VLA/20160409/vlamax_loc_r_qs.p", "rb"), encoding="latin1"
)
(
    TbLLr1,
    TbLLr2,
    ...
    ycLL90,
    timevla_all,
) = pickle.load(open("/media/rohit/VLA/20160409/MaxLL.p", "rb"), encoding="latin1")
...
xcrmax_all = np.hstack((np.array(qsx)[:, 0], xcrmax[1]))
ycrmax_all = np.hstack((np.array(qsy)[:, 0], ycrmax[1]))
...
def do_wavelet(Tbr, N):
    datawave_ = Tbr / 1.0e6
    datawave = datawave_[int(N / 2) : int(-1 * N / 2 + 1)] - np.convolve(
        datawave_, np.ones(N) / N, mode="valid"
    )
    std = np.std(datawave)
    datawave_std = datawave / std
    return datawave / std
...
if get_qs:
    ...
    for i in range(3):
        listqs = sorted(
            glob.glob(
                "/media/rohit/VLA/20160409/1844/fits_" + ss[i] + "_0-15/*spw.*FITS"
            )
        )[0:4799]
        listqs0 = sorted(
            glob.glob("/media/rohit/VLA/20160409/1844/fits_3_0-15/*spw.*FITS")
        )[0:4799]
        ts1 = [0] * 4799
        ts0 = [0] * 4799
        for k in range(4799):
            mr0, dr0, tr0 = get_evla_submap([listqs0[k]], 0, -1, 0, -1)
            ts0[k] = dr0[0][78:178, 78:178]
            mr1, dr1, tr1 = get_evla_submap([listqs[k]], 0, -1, 0, -1)
            ts1[k] = dr1[0][78:178, 78:178]
        ts1 = np.array(ts1)
        ts0 = np.array(ts0)
        cc0_s[i] = [0] * 100
        cc0_t[i] = [0] * 100
        for k1 in range(100):
            ...
            cc0_t[i][k1] = [0] * 100
            for k2 in range(100):
                tt1 = ts0[:, 135 - 78, 124 - 78] / np.sum(ts0[:, 135 - 78, 124 - 78])
                tt2 = ts1[:, k1, k2] / np.sum(ts1[:, k1, k2])
                tt1 = do_wavelet(tt1, 400)
                tt2 = do_wavelet(tt2, 400)
                cc0_s[i][k1][k2] = np.correlate(tt2, tt1, "same")
                cc0_s[i][k1][k2] = cc0_s[i][k1][k2] / np.max(cc0_s[i][k1][k2])
                t1 = ut.find_nearest(
                    cc0_s[i][k1][k2][2100:2200],
                    np.max(cc0_s[i][k1][k2][2100:2300]) * 0.4,
                )[0]
                t2 = ut.find_nearest(
                    cc0_s[i][k1][k2][2200:2300][::-1],
                    np.max(cc0_s[i][k1][k2][2100:2300]) * 0.4,
                )[0]
                cc0_t[i][k1][k2] = t2 - t1
    cc0_s = np.array(cc0_s).reshape(3, 100, 100, 4400)
    cc0_t = np.array(cc0_t).reshape(3, 100, 100)
...
if get_max:
    ...
    for i in range(1):
        ...
        for j in range(1):
            ...
            mr0, dr0, tr0 = get_evla_submap([listvla_r[0]], 0, -1, 0, -1)
            ts1 = [0] * 2000
            for k in range(2000):
                ts1[k] = [0] * 100
                mr1, dr1, tr1 = get_evla_submap([listvla_r[k]], 0, -1, 0, -1)
                ts1[k] = dr1[0][78:178, 78:178]
            ts1 = np.array(ts1)
            cc0_s[i][j] = [0] * 100
            cc0_t[i][j] = [0] * 100
            ...
            for k1 in range(100):
                ...
                for k2 in range(100):
                    ...
                    tt1 = ts1[:, 135 - 78, 124 - 78] / np.sum(
                        ts1[:, 135 - 78, 124 - 78]
                    )
                    tt2 = ts1[:, k1, k2] / np.sum(ts1[:, k1, k2])
                    tt1 = do_wavelet(tt1, 400)
                    tt2 = do_wavelet(tt2, 400)
                    cc0_s[i][j][k1][k2] = np.correlate(tt2, tt1, "same")
                    cc0_s[i][j][k1][k2] = cc0_s[i][j][k1][k2] / np.max(
                        cc0_s[i][j][k1][k2]
                    )
                    t1 = ut.find_nearest(
                        cc0_s[i][j][k1][k2][750:800],
                        np.max(cc0_s[i][j][k1][k2][750:850]) * 0.4,
                    )[0]
                    t2 = ut.find_nearest(
                        cc0_s[i][j][k1][k2][800:850][::-1],
                        np.max(cc0_s[i][j][k1][k2][750:850]) * 0.4,
                    )[0]
            ...
            sys.exit()
    cc0_s = np.array(cc0_s).reshape(32, 100, 100, 1601)
    cc0_t = np.array(cc0_t).reshape(32, 100, 100)
...
if get_max:
    ...
    for i in range(1):
        ...
        for j in range(1):
            ...
            listvla_r0 = sorted(
                glob.glob(
                    "/media/rohit/VLA/20160409/images_50ms_RR/spw_2/*spw.*32-47*.FITS"
                )
            )[0000:2000]
            ts1 = [0] * 2000
            ts0 = [0] * 2000
            for k in range(2000):
                mr1, dr1, tr1 = get_evla_submap([listvla_r[k]], 0, -1, 0, -1)
                ts1[k] = dr1[0][78:178, 78:178]
                mr0, dr0, tr0 = get_evla_submap([listvla_r0[k]], 0, -1, 0, -1)
                ts0[k] = dr0[0][78:178, 78:178]
            ts1 = np.array(ts1)
            ts0 = np.array(ts0)
            cc0_s[i][j] = [0] * 100
...
################ Read submaps ##############
allmaps = pickle.load(
    open("/media/rohit/VLA/20160409/20160409_submap_aia_50ms.p", "rb"),
    encoding="latin-1",
)
allmapsd = pickle.load(
    open("/media/rohit/VLA/20160409/20160409_submap_aia_diff_50ms.p", "rb"),
    encoding="latin-1",
)
allmapsb = pickle.load(
    open("/media/rohit/VLA/20160409/20160409_submap_aia_base_50ms.p", "rb"),
    encoding="latin-1",
)
# vla0 = pickle.load(
#    open("/media/rohit/VLA/20160409/20160409_vla_spw_0_50ms.p", "rb"),
#    encoding="latin-1",
# )
...
#########################
time94 = allmaps["aia94"]["time94"]
time131 = allmaps["aia131"]["time131"]
time335 = allmaps["aia335"]["time335"]
time1600 = allmaps["aia1600"]["time1600"]
time1700 = allmaps["aia1700"]["time1700"]
time171 = allmaps["aia171"]["time171"]
timed94 = allmapsd["aiad94"]["timed94"]
timed131 = allmapsd["aiad131"]["timed131"]
timed335 = allmapsd["aiad335"]["timed335"]
timed1600 = allmapsd["aiad1600"]["timed1600"]
timed1700 = allmapsd["aiad1700"]["timed1700"]
timed171 = allmapsd["aiad171"]["timed171"]
...
for i in range(m):
    ...
    tidx1600[i] = ut.find_predecessor(allmaps["aia1600"]["time1600"], timevla_all1[i])[
        0
    ]
    tidx1700[i] = ut.find_predecessor(allmaps["aia1700"]["time1700"], timevla_all1[i])[
        0
    ]
...
vlafreqidx = (
    np.array(list(np.arange(1)) * 2399).reshape(2399, 1).swapaxes(0, 1).flatten()
)
...
for k in range(len(lev)):
    ...
    for i in range(Tb0.shape[0]):
        ...
        for j in range(Tb0.shape[1]):
            bimage = ut.get_bimage(Tb0[i][j], lev[k] * 3.0e7 / np.nanmax(Tb0[i][j]))
...
maphmi = [0] * len(hmilist)
datahmi = [0] * len(hmilist)
...
hmifile = (
    "/media/rohit/VLA/20160409_EUV/hmi.m_45s.2016.04.09_18_45_00_TAI.magnetogram.fits"
)
hmimap = Map(hmifile)
...
b_hp_fr, b_hp_ls, b_hp_euv = pickle.load(
    open("/media/rohit/VLA/paraview/bproj.p", "rb")
)
...
if plot_dcp_hmi:
    for i in range(32):
        ...
        p = hmimap.plot(
            axes=ax0,
            extent=[xlaia, xraia, ylaia, yraia],
            aspect="auto",
            vmin=-2000,
            vmax=2000,
        )
        ...
        dcp_data = mapvla_v[i][0].data * 100 / mapvla_i[i][0].data
        dcp_data[np.isnan(dcp_data)] = 0
        dcp_data[np.where(dcp_data < 0)] = 0
        dcp_data[np.where(dcp_data > 100)] = 0
        dd0 = Map(dcp_data, mapvla_i[i][0].meta)
        xlvla = dd0.center.Tx.value - 2.0 * int(dd0.data.shape[0] / 2)
        xrvla = dd0.center.Tx.value + 2.0 * int(dd0.data.shape[0] / 2)
        ylvla = dd0.center.Ty.value - 2.0 * int(dd0.data.shape[1] / 2)
        yrvla = dd0.center.Ty.value + 2.0 * int(dd0.data.shape[0] / 2)
        dd0.draw_contours(
            levels=lev0, colors="r", linewidths=2, extent=[xlvla, xrvla, ylvla, yrvla]
        ...
        ax0.set_xlim([-850, -700])
        ax0.set_ylim([200, 350])
        ax0.text(
            -840,
            210,
            str(np.round(dd0.meta["crval3"] / 1.0e9, 4)) + " GHz",
            color="blue",
        )
        ax0.text(
            -840,
            330,
            "Contours (%): 50, 55, 60, 65, 70, 75, 80, 85, 90",
            color="yellow",
        )
...
if plot_vi_hmi:
    for i in range(32):
        ...
        mapvla_i[i][0].data[np.isnan(mapvla_i[i][0].data)] = 0
        mapvla_v[i][0].data[np.isnan(mapvla_v[i][0].data)] = 0
        mapvla_i[i][0].draw_contours(
            levels=lev1,
            colors="white",
            linewidths=2,
            extent=[xlvla, xrvla, ylvla, yrvla],
        )  # [xlpix,xrpix,ylpix,yrpix])
        mapvla_v[i][0].draw_contours(
            levels=lev1,
            colors="yellow",
            linewidths=2,
            extent=[xlvla, xrvla, ylvla, yrvla],
...
if plot_dcp_2d:
    ...
    ax0.plot(
        freq,
        maxTbv[:, 770:870].mean(axis=1) / maxTbi[:, 770:870].mean(axis=1) * 100,
        "o-",
    )
    ax0.errorbar(
        freq,
        maxTbv[:, 770:870].mean(axis=1) / maxTbi[:, 770:870].mean(axis=1) * 100,
        yerr=eTbv[:, 770] / maxTbi[:, 770] * 100,
    )
    ax0.set_ylabel("DCP (%)")
...
if plot_dcp:
    for i in range(32):
        f = plt.figure(figsize=(10, 10))
        ax0 = f.add_subplot(111)
        tidx_171 = ut.find_predecessor(
            allmaps["aia171"]["time171"], allmaps["aia171"]["time171"][0]
        )[0]
...
if plot_dcp:
    for i in range(2000):
        ...
        dcp_data = (
            ...
            * 100
            / mapvla_i[i].data
            * (mapvla_v[i][0].data / np.nanmax(mapvla_v[i][0].data))
        )
        dcp_data[np.isnan(dcp_data)] = 0
        dcp_data[np.where(dcp_data < 0)] = 0
        dcp_data[np.where(dcp_data > 100)] = 0
        ...
        mapvla_i[i].data[np.isnan(mapvla_i[i].data)] = 0
        mapvla_v[i][0].data[np.isnan(mapvla_v[i][0].data)] = 0
        ...
        ax1.plot(np.arange(2000) * 0.05, vlamax_v, "-", label="Stokes V")
        ax1.axvline(x=i * 0.05, color="k")
        ax1.set_xlabel("Time (sec)")
        ax1.set_ylabel("T$_{B}$")
        ax1.legend()
...
if plot_dcp:
    ...
    for i in range(800, 1200):
        ...
        # lev1=(1.5e7/dd0.data.max())*np.array([20,30,40,50,60,70,80,90])*u.percent
        lev1 = (
            np.array([30, 40, 50, 60, 70, 80, 90])
            / np.nanmax(mapvla_v[i][0].data)
            * (4.0e7)
            * u.percent
        )
        lev2 = np.array([1, 5, 10, 20, 50, 60, 70, 80, 90]) * u.percent
        ...
        dd0 = Map(dcp_data, mapvla_i[i].meta)
        lev0 = (
            np.array([50, 55, 60, 65, 70, 75, 80, 85, 90])
            / 100
            * np.nanmax(dd0.data)
            * u.percent
        )
        ...
        ax0.text(
            -840,
            330,
            "Contours (%): 20, 22, 24, 26, 28, 30, 32, 34, 36 MK",
            color="green",
        )
        ax1.plot(np.arange(2000) * 0.05, vlamax_v, "o-", label="Stokes V")
        ax1.axvline(x=i * 0.05, color="k")
        ax1.set_xlabel("Time (sec)")
        ax1.set_ylabel("T$_{B}$")
        ax1.legend()
...
for k in range(2000):
    ...
    Tbv_r1[k] = h.data[124:137, 121:124].mean()
    Tbv_r2[k] = h.data[138:141, 123:126].mean()
    Tbv[np.where(Tbv < Tbv.max() * 0.5)] = np.nan
    idx = np.where(
        np.abs(Tbv - np.nanmedian(Tbv)) == np.nanmin(np.abs(Tbv - np.nanmedian(Tbv)))
    )
    if len(idx[0]) == 1:
        yc, xc = idx[1], idx[0]
    else:
    ...
    yc, xc = np.where(Tbi == np.nanmax(Tbi))
    xcimax[k] = (
        hi.reference_coordinate.Tx.value
        + (xc - (hi.reference_pixel.x.value - 1)) * hi.scale.axis1.value
    )
    ycimax[k] = (
        hi.reference_coordinate.Ty.value
        + (yc - (hi.reference_pixel.y.value - 1)) * hi.scale.axis2.value
    )
    Tbi_r1[k] = hi.data[124:137, 121:124].mean()
    Tbi_r2[k] = hi.data[138:141, 123:126].mean()
    Tbi[np.where(Tbi < Tbi.max() * 0.5)] = np.nan
    idx = np.where(
        np.abs(Tbi - np.nanmedian(Tbi)) == np.nanmin(np.abs(Tbi - np.nanmedian(Tbi)))
    )
    if len(idx[0]) == 1:
        yc, xc = idx[1], idx[0]
    else:
    ...
    Tbl_r1[k] = hl.data[124:137, 121:124].mean()
    Tbl_r2[k] = hl.data[138:141, 123:126].mean()
    hr = mapvla_r[k][0]
    Tbr = hr.data
    yc, xc = np.where(Tbl == np.nanmax(Tbl))
    xclmax[k] = (
    ...
    Tbr_r1[k] = hi.data[124:137, 121:124].mean()
    Tbr_r2[k] = hi.data[138:141, 123:126].mean()
...
if plot_tb:
    ...
    for i in range(32):
        ...
        ax1 = ax[1]
        ax0.plot(
            np.arange(2000) * 0.05, np.array(Tbi_r1[i]) / 1.0e6, "-", label="R1 (North)"
        )  #
        ax0.plot(
            np.arange(2000) * 0.05, np.array(Tbi_r2[i]) / 1.0e6, "-", label="R2 (South)"
        )
        ax0.legend()
        ax1.plot(
            np.arange(2000) * 0.05,
            (np.array(Tbi_r2[i]) - np.array(Tbi_r1[i])) / 1.0e6,
            "-",
            label="R2-R1",
        )
        ax1.legend()
        ...
        ax0.set_ylim(0, 50)
        ax0.legend()
        ax0.set_title(str(freq[i]) + " GHz")
        ax1.set_ylabel("$T_B$ (MK)")
        ax0.set_ylabel("T$_B$ (MK)")
        ax1.set_xlabel("time (sec)")
        plt.savefig(
            "/media/rohit/VLA/20160409/pngs_spec/Tbi_regions_" + "%02d" % i + ".png"
        )
        plt.close()
...
if plot_tb:
    ...
    for i in range(32):
        ...
        ax1.plot(
            np.arange(2000) * 0.05,
            (np.array(Tbv_r1[i]) / np.array(Tbi_r1[i])) * 100,
            "-",
            label="R1 (North)",
        )
        ax1.legend()
        ax1.plot(
            np.arange(2000) * 0.05,
            (np.array(Tbv_r2[i]) / np.array(Tbi_r2[i])) * 100,
            "-",
            label="R2 (South)",
        )
        ax1.legend()
        ...
        ax0.legend()
        ax0.set_title(str(freq[i]) + " GHz")
        ax1.set_ylabel("DCP (V/I) (\%)")
        ax0.set_ylabel("T$_b$ (MK)")
        ax1.set_xlabel("time (sec)")
        plt.savefig(
            "/media/rohit/VLA/20160409/pngs_spec/Tbi_dcp_regions_" + "%02d" % i + ".png"
        )
        plt.close()
    i = 0
    for i in range(2000):
        ...
        ax1 = ax[1]
        ax0.plot(
            np.arange(2000) * 0.05, np.array(Tbi_r1[0]) / 1.0e6, "-", label="R1 (North)"
        )  #
        ax0.plot(
            np.arange(2000) * 0.05, np.array(Tbi_r2[0]) / 1.0e6, "-", label="R2 (South)"
        )
        ax0.legend()
        ...
        ax0.set_ylim(0, 50)
        ax0.legend()
        ax0.set_title(str(freq[0]) + " GHz")
        ax1.set_ylabel("DCP (V/I) (\%)")
        ax0.set_ylabel("T$_b$ (MK)")
        ax0.set_xlabel("time (sec)")
        ax1.set_xlabel("Frequency (MHz)")
        plt.savefig(
            "/media/rohit/VLA/20160409/pngs_spec/dcp_spec_regions_"
            + "%02d" % i
            + ".png"
        )
        plt.close()
    ...
    ax0 = ax[0]
    ax1 = ax[1]
    ax0.plot(np.arange(2000) * 0.05, np.array(Tb_r1) / 1.0e6, "o-", label="R1")
    ax0.plot(np.arange(2000) * 0.05, np.array(Tb_r2) / 1.0e6, "o-", label="R2")
    ax0.legend()
    ax1.plot(
        np.arange(2000) * 0.05,
        (np.array(Tb_r2) - np.array(Tb_r1)) / 1.0e6,
        "o-",
        label="R2-R1",
    )
    ax1.legend()
    ...
    ax1.plot(np.arange(len(y)) * 0.05, np.hstack((qsTbr_r1, Tbr_r1[1])) / 1.0e6, "o-")
    ax1.set_ylabel("$T_B$ (MK)")
    ax0.set_ylabel("Y-Coordinate (arcsec)")
    ax1.set_xlabel("Time (sec)")
    plt.show()
    ...
    ax1.plot(np.arange(2000) * 0.05, vlamax_v / 1.0e6, "o-")
    ax1.set_ylabel("$T_B$ (MK)")
    ax0.set_ylabel("Y-Coordinate (arcsec)")
    ax1.set_xlabel("Time (sec)")
    plt.show()

idx1 = np.where(ycrmax_all < 255)[0]
idx2 = np.where(ycrmax_all > 255)[0]
t = np.arange(len(ycrmax_all)) * 0.05
t1 = t[idx1]
ycrmax1 = ycrmax_all[idx1]
t2 = t[idx2]
ycrmax2 = ycrmax_all[idx2]
xcrmax1 = xcrmax_all[idx1]
t2 = t[idx2]
xcrmax2 = xcrmax_all[idx2]
vlamax_r1 = Tbr1_rall[idx1]
vlamax_r2 = Tbr1_rall[idx2]
f, ax = plt.subplots(3, 1, figsize=(8, 15), sharex=True)
ax0 = ax[0]
ax1 = ax[1]
ax2 = ax[2]
ax0.plot(t1, ycrmax1, "o", color="r")
ax0.plot(t2, ycrmax2, "o", color="g")
ax1.plot(t1, xcrmax1, "o", color="r")
ax1.plot(t2, xcrmax2, "o", color="g")
ax2.plot(t, Tbr1_rall / 1.0e6, "-", color="k")
ax2.plot(t1, vlamax_r1 / 1.0e6, "o", markersize=8, color="r")
ax2.plot(t2, vlamax_r2 / 1.0e6, "o", markersize=8, color="g")
ax1.set_ylim(-750, -800)
ax2.set_ylabel("$T_B$ (MK)")
ax0.set_ylabel("Y-Coordinate (arcsec)")
...
yraia = 1031.9
p = hmimap.plot(
    axes=ax0, extent=[xlaia, xraia, ylaia, yraia], aspect="auto", vmin=-2000, vmax=2000
)
...
ax1.plot(t, vlamax_v, "o-", label="Stokes V")
ax1.axvline(x=i * 0.05, color="k")
ax1.set_xlabel("Time (sec)")
ax1.set_ylabel("T$_{B}$")
ax1.legend(loc=4)
...
yraia = 369.4
p = mapex_bz[0].plot(
    axes=ax0, extent=[xlaia, xraia, ylaia, yraia], aspect="auto", vmin=-750, vmax=750
)
f.colorbar(p, label="B (G)")
ax0.plot(xcvmax[781], ycvmax[781], "o", color="g")
ax0.plot(xcvmax[858], ycvmax[858], "o", color="g")
...
TbLLr1 = [0] * len(vlalistLL)
TbLLr2 = [0] * len(vlalistLL)
TbLLr3 = [0] * len(vlalistLL)
TbLLr4 = [0] * len(vlalistLL)
TbLLr5 = [0] * len(vlalistLL)
xcLLr1 = [0] * len(vlalistLL)
xcLLr2 = [0] * len(vlalistLL)
xcLLr3 = [0] * len(vlalistLL)
xcLLr4 = [0] * len(vlalistLL)
xcLLr5 = [0] * len(vlalistLL)
ycLLr1 = [0] * len(vlalistLL)
ycLLr2 = [0] * len(vlalistLL)
ycLLr3 = [0] * len(vlalistLL)
ycLLr4 = [0] * len(vlalistLL)
ycLLr5 = [0] * len(vlalistLL)
TbRRr1 = [0] * len(vlalistRR)
TbRRr2 = [0] * len(vlalistRR)
TbRRr3 = [0] * len(vlalistRR)
TbRRr4 = [0] * len(vlalistRR)
TbRRr5 = [0] * len(vlalistRR)
xcRRr1 = [0] * len(vlalistRR)
xcRRr2 = [0] * len(vlalistRR)
xcRRr3 = [0] * len(vlalistRR)
xcRRr4 = [0] * len(vlalistRR)
xcRRr5 = [0] * len(vlalistRR)
ycRRr1 = [0] * len(vlalistRR)
ycRRr2 = [0] * len(vlalistRR)
ycRRr3 = [0] * len(vlalistRR)
ycRRr4 = [0] * len(vlalistRR)
ycRRr5 = [0] * len(vlalistRR)
xcRRmax = [0] * len(vlalistRR)
ycRRmax = [0] * len(vlalistRR)
xcLLmax = [0] * len(vlalistLL)
ycLLmax = [0] * len(vlalistLL)
xcRR90 = [0] * len(vlalistRR)
ycRR90 = [0] * len(vlalistRR)
xcLL90 = [0] * len(vlalistLL)
ycLL90 = [0] * len(vlalistLL)
...
for vfile in vlalistRR:
    ...
    v = v + 1
TbLLr1 = np.array(TbLLr1)
TbLLr2 = np.array(TbLLr2)
TbLLr3 = np.array(TbLLr3)
TbLLr4 = np.array(TbLLr4)
TbLLr5 = np.array(TbLLr5)
TbRRr1 = np.array(TbRRr1)
TbRRr2 = np.array(TbRRr2)
TbRRr3 = np.array(TbRRr3)
TbRRr4 = np.array(TbRRr4)
TbRRr5 = np.array(TbRRr5)
xcRRmax = np.array(xcRRmax)
ycRRmax = np.array(ycRRmax)
xcLLmax = np.array(xcLLmax)
ycLLmax = np.array(ycLLmax)
xcLL90 = np.array(xcLL90)
ycLL90 = np.array(ycLL90)
xcRR90 = np.array(xcRR90)
ycRR90 = np.array(ycRR90)
...
pickle.dump(
    [TbRRr1, TbRRr2, TbRRr3, TbRRr4, TbRRr5, xcRRmax, ycRRmax, xcRR90, ycRR90],
    open("MaxRR.p", "wb"),
)

#### READ

TbRRr1, TbRRr2, TbRRr3, TbRRr4, TbRRr5, xcRRmax, ycRRmax, xcRR90, ycRR90 = pickle.load(
    open("/media/rohit/VLA/20160409/MaxRR.p", "rb"), encoding="latin1"
)
(
    TbLLr1,
    TbLLr2,
...
ax1 = ax.twinx()
ax1.plot(
    np.linspace(0, 120, 2399), TbRRr1[:, 0] / 1.0e6, "o-", color="black", label="T$_B$"
)
ax.plot(
    np.linspace(0, 120, 2399), ycRR90.std(axis=1), "o-", color="red", label="Y-Centroid"
)
ax.set_xlabel("Time (sec)")
ax.set_ylabel("Y-Centroid (arcsec)")
ax1.set_ylabel("T$_B$ (MK)")
ax.legend()
ax1.legend(loc=2)
plt.show()
...
ax.plot(
    np.linspace(0, 120, 2399),
    xcRR90.mean(axis=1) - xcLL90.mean(axis=1),
    "o-",
    color="red",
    label="X-Centroid (RR-LL)",
)
ax.set_xlabel("Time (sec)")
ax.set_ylabel("X-Centroid (arcsec)")
ax1.set_ylabel("T$_B$ (MK)")
ax.legend()
ax1.legend(loc=2)
plt.show()
...
for ff in range(32):
    loc_diff_LR = np.sqrt(
        (np.concatenate(xcLLmax[:, ff])[:2399] - xcRRmax[:2399, ff, 0]) ** 2
        + (np.concatenate(ycLLmax[:, ff])[:2399] - ycRRmax[:2399, ff, 0]) ** 2
    )
    fig, ax = plt.subplots(3, 1, sharex=True, figsize=(10, 13))
    ax[0].plot(
        x,
        (TbLLr1[:, ff] + TbRRr1[:, ff]) / 1.0e6,
        "o-",
        label=str(np.round(freq[ff], 3)) + " GHz",
    )
...
im1 = ax[0, 0].imshow(
    TbLLr1.swapaxes(0, 1), aspect="auto", origin=0, vmin=1.0e6, vmax=10.0e6
)
ax[1, 0].imshow(TbLLr5.swapaxes(0, 1), aspect="auto", origin=0, vmin=1.0e6, vmax=10.0e6)
ax[0, 1].imshow(TbLLr3.swapaxes(0, 1), aspect="auto", origin=0, vmin=1.0e6, vmax=10.0e6)
ax[1, 1].imshow(TbLLr4.swapaxes(0, 1), aspect="auto", origin=0, vmin=1.0e6, vmax=10.0e6)
...
ax[0, 0].set_xticks([0, 480, 960, 1440, 1920, 2400])
ax[0, 0].set_xticklabels(["0", "24", "48", "72", "98", "120"])
ax[0, 1].set_xticks([0, 480, 960, 1440, 1920, 2400])
ax[0, 1].set_xticklabels(["0", "24", "48", "72", "98", "120"])
ax[1, 0].set_xticks([0, 480, 960, 1440, 1920, 2400])
ax[1, 0].set_xticklabels(["0", "24", "48", "72", "98", "120"])
ax[1, 1].set_xticks([0, 480, 960, 1440, 1920, 2400])
ax[1, 1].set_xticklabels(["0", "24", "48", "72", "98", "120"])
ax[0, 0].set_yticks(np.arange(32)[::4])
ax[0, 0].set_yticklabels(np.round(np.linspace(0.994, 2.006, 32)[::4], 2))
ax[1, 0].set_yticks(np.arange(32)[::4])
ax[1, 0].set_yticklabels(np.round(np.linspace(0.994, 2.006, 32)[::4], 2))
ax[0, 1].set_yticks(np.arange(32)[::4])
ax[0, 1].set_yticklabels(np.round(np.linspace(0.994, 2.006, 32)[::4], 2))
ax[1, 1].set_yticks(np.arange(32)[::4])
ax[1, 1].set_yticklabels(np.round(np.linspace(0.994, 2.006, 32)[::4], 2))
...
for i in range(2399):
    ...
    cc = allmaps["aia171"]["map171"][tidx_171]
    xlaia = cc.center.Tx.value - 0.61 * int(cc.data.shape[0] / 2)
    xraia = cc.center.Tx.value + 0.61 * int(cc.data.shape[0] / 2)
    ylaia = cc.center.Ty.value - 0.61 * int(cc.data.shape[1] / 2)
    yraia = cc.center.Ty.value + 0.61 * int(cc.data.shape[0] / 2)
    p = cc.plot(axes=ax0, extent=[xlaia, xraia, ylaia, yraia], aspect="auto")
    # lev1=(1.5e7/dd0.data.max())*np.array([20,30,40,50,60,70,80,90])*u.percent
    lev0 = np.array([40, 50, 60, 70, 90]) * u.percent
    dd0 = vmLL
    dd0.data[np.isnan(dd0.data)] = 0
    xlvla = dd0.center.Tx.value - 2.0 * int(dd0.data.shape[0] / 2)
    xrvla = dd0.center.Tx.value + 2.0 * int(dd0.data.shape[0] / 2)
    ylvla = dd0.center.Ty.value - 2.0 * int(dd0.data.shape[1] / 2)
    yrvla = dd0.center.Ty.value + 2.0 * int(dd0.data.shape[0] / 2)
    ...
    dd1 = vmRR
    dd1.data[np.isnan(dd1.data)] = 0
    xlvla = dd1.center.Tx.value - 2.0 * int(dd1.data.shape[0] / 2)
    xrvla = dd1.center.Tx.value + 2.0 * int(dd1.data.shape[0] / 2)
    ylvla = dd1.center.Ty.value - 2.0 * int(dd1.data.shape[1] / 2)
    yrvla = dd1.center.Ty.value + 2.0 * int(dd1.data.shape[0] / 2)
    dd1.draw_contours(
        levels=lev1, colors="b", linewidths=2, extent=[xlvla, xrvla, ylvla, yrvla]
    )  # [xlpix,xrpix,ylpix,yrpix])
    # ax0.set_title('AIA 171 $\AA$:'+timstr_171[tidx171[i]]+' VLA: '+timstr_vla[i]+' UT')
    # ax0.set_xlim([xlaia,xraia]);ax0.set_ylim([ylaia,yraia])
    ax0.set_xlim([-1000, -700])
    ax0.set_ylim([50, 400])
    ax0.text(
        -1000, 60, str(np.round(dd0.meta["crval3"] / 1.0e9, 4)) + " GHz", color="white"
    )
    ax0.add_patch(
        patches.Rectangle(
            (-781, 233), 40, 40, linewidth=5, edgecolor="cyan", facecolor="none"
        )
    )
    ax0.add_patch(
        patches.Rectangle(
            (-841, 265), 20, 20, linewidth=5, edgecolor="g", facecolor="none"
        )
    )
    ax0.add_patch(
        patches.Rectangle(
            (-881, 175), 40, 40, linewidth=5, edgecolor="red", facecolor="none"
        )
    )
    ax0.add_patch(
        patches.Rectangle(
            (-961, 195), 40, 40, linewidth=5, edgecolor="b", facecolor="none"
        )
    )
    ax0.add_patch(
        patches.Rectangle(
            (-831, 175), 40, 40, linewidth=5, edgecolor="magenta", facecolor="none"
        )
    )
    ax1 = f.add_subplot(212)
    ax1.plot(
        freq, TbLLr1[i] / 1.0e6, "o-", markersize=2, label="Region 1 (LL)", color="cyan"
    )
    ax1.plot(
        freq,
        TbLLr2[i] / 1.0e6,
    ...
    ax1.plot(
        freq, TbLLr3[i] / 1.0e6, "o-", markersize=2, label="Region 3 (LL)", color="red"
    )
    ax1.plot(
        freq, TbLLr4[i] / 1.0e6, "o-", markersize=2, label="Region 4 (LL)", color="blue"
    )
    ax1.plot(
        freq,
        TbLLr5[i] / 1.0e6,
    ...
    ax1.plot(
        freq, TbRRr3[i] / 1.0e6, "o--", markersize=2, label="Region 3 (RR)", color="red"
    )
    ax1.plot(
        freq,
        TbRRr4[i] / 1.0e6,
...
for i in range(1):
    ...
    yrvla = dd1.center.Ty.value + 2.0 * int(dd1.data.shape[0] / 2)
    dd1.draw_contours(
        levels=lev1, colors="white", linewidths=2, extent=[xlvla, xrvla, ylvla, yrvla]
    )  # [xlpix,xrpix,ylpix,yrpix])
    # ax0.set_title('AIA 171 $\AA$:'+timstr_171[tidx171[i]]+' VLA: '+timstr_vla[i]+' UT')
    # ax0.set_xlim([xlaia,xraia]);ax0.set_ylim([ylaia,yraia])
    ax0.set_xlim([-900, -700])
    ...
    ax1 = f.add_subplot(212)
    ax1.plot(
        Tbi_r1[0:8].mean(axis=0) / 1.0e6,
        "o-",
        markersize=2,
        label="Region 1 (Stokes-I)",
        color="k",
    )
    ax1.legend(fontsize=10)  # ;ax1.set_ylim(0,100)
    ...
    ax1.set_ylim(0, 50)
    ax1.grid(True)
    ax1.set_xticks([0, 600, 1200, 1800])
    ax1.set_xticklabels(["18:44:00", "18:44:30", "18:45:00", "18:45:30"])
    ax1.set_xlabel("Time (HH:MM:SS UT)")
    ax1.set_ylabel("$T_B$ (MK)")
    # plt.savefig('pngs_spec_movie/aia171_'+"%02d"%i+'.png')
    plt.show()
    plt.figure(figsize=(5, 5))
    plt.imshow(Tbi_r1[0:16] / 1.0e6, aspect="auto", origin=0, cmap="YlOrRd")
...
vlaLL = pickle.load(open(vlafileLL, "rb"), encoding="latin1")
vlaRR = pickle.load(open(vlafileRR, "rb"), encoding="latin1")
tidx_94 = ut.find_predecessor(allmaps["aia94"]["time94"], vlaLL["vla"]["timevla"][0])[0]
tidx_131 = ut.find_predecessor(
    allmaps["aia131"]["time131"], vlaLL["vla"]["timevla"][0]
)[0]
tidx_171 = ut.find_predecessor(
    allmaps["aia171"]["time171"], vlaLL["vla"]["timevla"][0]
)[0]
tidx_193 = ut.find_predecessor(
    allmaps["aia193"]["time193"], vlaLL["vla"]["timevla"][0]
)[0]
tidx_1600 = ut.find_predecessor(
    allmaps["aia1600"]["time1600"], vlaLL["vla"]["timevla"][0]
)[0]
...
for i in range(32):
    ...
    # ax0.set_title('AIA 171 $\AA$:'+timstr_171[tidx171[i]]+' VLA: '+timstr_vla[i]+' UT')
    ax0.set_xlim([xlaia, xraia])
    ax0.set_ylim([ylaia, yraia])
    ax0.text(
        -1200, 0, str(np.round(dd0.meta["crval3"] / 1.0e9, 4)) + " GHz", color="green"
    )
...
################ Make Time String ##########
listvla = sorted(
    glob.glob("/media/rohit/VLA/20160409/images_50ms_LL/spw_0/sun*.spw.0_16-31*FITS")
)
...
timstr_vla = [0] * len(listvla)
timstr_1600 = [0] * len(list1600)
timstr_171 = [0] * len(list171)
...
################ FERMI & GOES & VLA DS #######################
goes = readsav("/data/Dropbox/20160409/idlsave_goes_20160409.sav")
# goes = readsav("/home/rohit/idlsave_goes.sav")
...
fm0 = fm[0]
fm1 = fm[1]
fmrate = fm1.data["RATE"]
fmtime = fm1.data["TIME"]
...
############### MAKE DS #####################
ds1 = np.load(
    "/media/rohit/VLA/20160409/sun_L_20160409T184000-184400UT.50ms.cal.ms.dspec.npz"
)
specfile = (
    "/media/rohit/VLA/20160409/sun_L_20160409T1844-1846UT.50ms.cal.ms.dspec.median.npz"
)
...
for i in range(8):
    tim[i] = np.hstack(
        (tim_[2089 * i : 2089 * (i + 1)], tim_[16712 + i * 311 : 16712 + (i + 1) * 311])
    )
tim = np.array(tim)
ds_ = ds_[:, 0, :, :]
ds = [0] * 2
for i in range(2):
    ...
    for j in range(64):
        ...
        for k in range(8):
            ds[i][j][k] = np.hstack(
                (
                    ds_[i, j, 2089 * k : 2089 * (k + 1)],
                    ds_[i, j, 16712 + k * 311 : 16712 + (k + 1) * 311],
                )
            )
...
freq = freq.swapaxes(0, 1).flatten() / 1.0e9
ds1s = (
    np.subtract(ds1["spec"][0][0].swapaxes(0, 1), ds1["spec"][0][0][:, 0]).swapaxes(
        0, 1
    )
    / 3
)
ds_LL1 = np.hstack(
    (
        ds1s,
        np.subtract(ds1s[:, -1], -1 * ds_LL[6:512].swapaxes(0, 1) + 28).swapaxes(0, 1),
    )
)
ds_RR1 = np.hstack(
    (
        ds1s,
        np.subtract(ds1s[:, -1], -1 * ds_RR[6:512].swapaxes(0, 1) + 28).swapaxes(0, 1),
    )
)
...
plot_euv_time = 1
if plot_euv_time:
    dsplt = ds_LL1[0:128].mean(axis=0) / 120
    dsplt[2130:2170] = np.nan
    dsplt[4530:4570] = np.nan
    dsplt[6930:6970] = np.nan
    ds_I = (ds_LL1 + ds_RR1) / 350 * 20
    ds_I[2130:2170] = np.nan
    ds_I[4530:4570] = np.nan
    ds_I[6930:6970] = np.nan
    ds_I[265:295] = 0
    ...
    im0 = ax[0].imshow(
        ...
        vmin=0.1,
        vmax=10,
        extent=[67200.0, 67560.0, freq[0], freq[-1]],
    )
    ...
    ax[1].axvline(x=67464, linestyle="--", color="gray")
    ax[1].axvline(x=67484, linestyle="--", color="gray")
    ax[1].text(67400, 2.5, "18:43:35", color="blue")
    ax[1].text(67320, 2.4, "18:41:55", color="blue")
    ax[1].text(67420, 2.3, "18:43:50", color="blue")
    ax[1].text(67443, 2.5, "18:44:03", color="blue")
    ax[1].text(67484, 2.5, "18:44:44", color="blue")
    ax[1].text(67462, 2.2, "18:44:24", color="blue")
    ax[1].plot(time94, ts94 - ts94[1] + 0.5, "o-", markersize=2, label="AIA 94 $\AA$")
    ax[1].plot(
        time131, ts131 - ts131[1] + 0.5, "o-", markersize=2, label="AIA 131 $\AA$"
    )
    ax[1].plot(
        time335, ts335 - ts335[1] + 0.5, "o-", markersize=2, label="AIA 335 $\AA$"
    )
    ax[1].plot(
        time1600, ts1600 - ts1600[1] + 0.5, "o-", markersize=2, label="AIA 1600 $\AA$"
    )
    ax[1].plot(
        qstimevla[0][0] + np.arange(7200) * 0.05,
        dsplt,
        "-",
        color="k",
        alpha=0.8,
    ...
    ax[1].plot(
        gtime[292:585] + 66600 + 38,  # Adding delay
        gf0540[292:585] / np.nanmax(gf0540[292:585]),
        "-",
        color="cyan",
        label="Time shifted GOES (0.5-4.0$\AA$)",
    )
    ax[1].legend(loc=2, prop={"size": 12})
    ax[2].plot(
        gtime[292:585] + 66600,
        gf0540[292:585] / 1.0e-8,
        ".-",
        color="g",
        label="GOES (0.5-4.0$\AA$)",
    )
    ax2 = ax[2].twinx()
    ax2.plot(
        gtime[292:585] + 66600,
        gf1080[292:585] / 1.0e-6,
        ".-",
        label="GOES (1.0-8.0$\AA$)",
    )
    ax[2].legend(loc=1)
    ax[1].set_ylabel("Arbitrary Amplitude")
    ax[2].set_xlabel("Time (HH:MM UT)")  # ;ax1.set_ylabel('')
    ax[2].set_xlim([67200.0, 67560.0])
    ax2.set_ylabel("Fermi Count Rates (Arbitrary)")
    ax[2].set_ylabel("GOES Flux ($j\\times10^{-8}$W/m$^2$)")
    ax[1].text(67530, 3.4, "(B)", color="white", fontsize=20)
    ax[1].text(67530, 2.3, "(C)", color="k", fontsize=20)
    ax[2].axvline(x=67415, linestyle="--", color="gray", linewidth=5)
    ax[2].text(67530, 1.6, "(D)", color="k", fontsize=20)
    ax2.plot(
        fmtime[14244:14332],
        fmrate[14244:14332:, 1:8].mean(axis=1) / 200,
        "o-",
        markersize=2,
        color="brown",
        label="FERMI (5-12 keV)",
    )
    ax2.plot(
        fmtime[14244:14332],
        fmrate[14244:14332:, 14:21].mean(axis=1) / 20,
        "o-",
        markersize=2,
        color="magenta",
        label="FERMI (18-28 keV)",
    )
    ax2.legend(loc=2)
    plt.show()
...
# ----- DS- Yu plot
Lband_DS = np.load(
    "/data/Dropbox/20160409/sun_L_20160409.4s.cal.ms.dspec.median/spec.npy"
)
Sband_DS = np.load(
    "/data/Dropbox/20160409/sun_S_20160409.4s.cal.ms.dspec.quack/spec.npy"
)
Lband_freq = np.load(
    "/data/Dropbox/20160409/sun_L_20160409.4s.cal.ms.dspec.median/freq.npy"
)
Lband_time = np.load(
    "/data/Dropbox/20160409/sun_L_20160409.4s.cal.ms.dspec.median/tim.npy"
)

df = 90
f, ax = plt.subplots(1, 1)
im0 = ax.imshow(Lband_DS[0][0] / 100.0, origin="lower", aspect="auto", cmap="coolwarm")
ax.axvline(x=600 + df, linestyle="-", color="yellow", linewidth=5)
ax.axvline(x=690 + df, linestyle="-", color="yellow", linewidth=5)
ax.set_xticks(np.array([0, 900, 1800, 2700, 3600, 4500]) + df)
ax.set_xticklabels(["18:00", "19:00", "20:00", "21:00", "22:00", "23:00"])
ax.set_yticks([3, 103, 203, 309, 409, 509])
ax.set_yticklabels([freq[3], freq[103], freq[203], freq[309], freq[409], freq[509]])
ax.set_ylabel("Frequency (GHz)")
ax.set_xlabel("Time (HH:MM UT)")
divider = make_axes_locatable(ax)
cax = divider.append_axes("right", size="0.5%", pad=0.01)
f.colorbar(im0, cax=cax, orientation="vertical", label="(SFU)")
ax.text(4000, 400, "(A)", color="white", fontsize=20)
ax.text(800, 400, "Time Duration (18:40 to 18:46 UT)", color="yellow", fontsize=15)
plt.show()

# -----
f, ax = plt.subplots(1, 1)
ax.plot(gtime - 600 + 67200, gf0540 / 1.0e-8, "o-", color="g", label="0.5-4.0$\AA$")
ax1 = ax.twinx()
ax1.plot(gtime - 600 + 67200, gf1080 / 1.0e-6, "o-", color="b", label="1.0-8.0$\AA$")
...
ax.set_ylabel("Flux ($\\times10^{-23}$W/m$^2$)")
ax1.set_ylabel("Flux ($\\times10^{-23}$W/m$^2$)")
ax.set_xlabel("Time (HH:MM UT)")
ax.legend(loc=4)
ax1.legend(loc=2)
plt.show()
...
listvla_rr = sorted(
    glob.glob("/media/rohit/VLA/20160409/images_50ms_RR/spw_0/*spw.0_16-31*FITS")
)
v = Map(listvla_rr[860])
v.data[np.isnan(v.data)] = 0
listvla_rr = sorted(
    glob.glob("/media/rohit/VLA/20160409/images_50ms_RR/spw_2/*spw.2_16-31*FITS")
)
v2 = Map(listvla_rr[860])
v2.data[np.isnan(v2.data)] = 0
listvla_rr = sorted(
    glob.glob("/media/rohit/VLA/20160409/images_50ms_RR/spw_4/*spw.4_16-31*FITS")
)
v4 = Map(listvla_rr[860])
v4.data[np.isnan(v4.data)] = 0
listvla_rr = sorted(
    glob.glob("/media/rohit/VLA/20160409/images_50ms_RR/spw_6/*spw.6_16-31*FITS")
)
v6 = Map(listvla_rr[860])
v6.data[np.isnan(v6.data)] = 0
# ---------------
f = plt.figure(figsize=(10, 10))
dd = v
dd.data[np.isnan(dd.data)] = 0
...
yraia = cc.center.Ty.value + 0.61 * int(cc.data.shape[0] / 2)
p = cc.plot(
    axes=ax0, aspect="auto", norm=matplotlib.colors.Normalize(vmin=1.0e2, vmax=2.0e3)
)
# lev1=(1.5e7/dd.data.max())*np.array([60,70,80,90])*u.percent
lev1 = np.array([60, 70, 80, 90]) * u.percent
xlvla = dd.center.Tx.value - 2.0 * int(dd.data.shape[0] / 2)
...
dd.draw_contours(
    axes=ax0, levels=lev1, colors="r", linewidths=2, extent=[xlvla, xrvla, ylvla, yrvla]
)  # [xlpix,xrpix,ylpix,yrpix])
...
dd.data[np.isnan(dd.data)] = 0
cc = allmaps["aia171"]["map171"][10]
d = SkyCoord(xci90 * u.arcsec, yci90 * u.arcsec, frame=cc.coordinate_frame)
pix_x90, pix_y90 = cc.world_to_pixel(d)
pix_x90 = pix_x90.value
pix_y90 = pix_y90.value
f = plt.figure(figsize=(10, 10))
ax0 = f.add_subplot(111, projection=cc)
xlaia = cc.center.Tx.value - 0.61 * int(cc.data.shape[0] / 2)
xraia = cc.center.Tx.value + 0.61 * int(cc.data.shape[0] / 2)
...
yrvla = dd.center.Ty.value + 2.0 * int(dd.data.shape[0] / 2)
m=50
ax0.plot(pix_x90[0][::m], pix_y90[0][::m], "*", color="yellow", alpha=0.3, label="0.99 GHz")
ax0.plot(pix_x90[4][::m], pix_y90[4][::m], "o", color="magenta", alpha=0.3, label="1.12 GHz")
ax0.plot(pix_x90[8][::m], pix_y90[8][::m], "s", color="lime", alpha=0.3, label="1.26 GHz")
l0 = ax0.legend()
...
ax0.hist(xci90[0],bins=30,histtype='step',linestyle='dotted',linewidth=3,density=True,label="0.99 GHz",color='darkkhaki')
ax0.hist(xci90[4],bins=30,histtype='step',linestyle='dashed',linewidth=3,density=True,label="1.12 GHz",color='magenta')
ax0.hist(xci90[8],bins=30,histtype='step',density=True,linewidth=3,label="1.26 GHz",color='lime')
ax1.hist(yci90[0],bins=30,histtype='step',linestyle='dotted',linewidth=3,density=True,label="0.99 GHz",color='darkkhaki')
ax1.hist(yci90[4],bins=30,histtype='step',linestyle='dashed',linewidth=3,density=True,label="1.12 GHz",color='magenta')
ax1.hist(yci90[8],bins=30,histtype='step',density=True,linewidth=3,label="1.26 GHz",color='lime')
...
yrvla = dd.center.Ty.value + 2.0 * int(dd.data.shape[0] / 2)
dd.draw_contours(
    levels=lev1, colors="r", linewidths=2, extent=[xlvla, xrvla, ylvla, yrvla]
)  # [xlpix,xrpix,ylpix,yrpix])
ax0.set_title("AIA 171 $\AA$:18:42:10 UT VLA: 18:44:43.00-18:44:43.05 UT")
# ax0.set_xlim([xlaia,xraia]);ax0.set_ylim([ylaia,yraia])
...
yrvla = dd.center.Ty.value + 2.0 * int(dd.data.shape[0] / 2)
dd.draw_contours(
    levels=lev1, colors=color_array[0][0:3], linewidths=2
)  # ,extent=[xlvla,xrvla,ylvla,yrvla])#[xlpix,xrpix,ylpix,yrpix])
v2.draw_contours(
    levels=lev1, colors=color_array[30][0:3], linewidths=2
)  # ,extent=[xlvla,xrvla,ylvla,yrvla])#[xlpix,xrpix,ylpix,yrpix])
v4.draw_contours(
    levels=lev1, colors=color_array[40][0:3], linewidths=2
)  # ,extent=[xlvla,xrvla,ylvla,yrvla])#[xlpix,xrpix,ylpix,yrpix])
v6.draw_contours(
    levels=lev1, colors=color_array[50][0:3], linewidths=2
)  # ,extent=[xlvla,xrvla,ylvla,yrvla])#[xlpix,xrpix,ylpix,yrpix])
p = ee.plot(
    axes=ax0, aspect="auto", cmap="rainbow_alpha", vmin=100, vmax=8000, alpha=0.7
)  # ,extent=[xlaia,xraia,ylaia,yraia])
ax0.set_title("HMI: 18:50:47 UT VLA: 18:44:43.00-18:44:43.05 UT")
ax0.text(
    400,
    100,
    str(np.round(dd.meta["restfrq"] / 1.0e9, 3)) + " GHz",
    fontsize=18,
    color=color_array[0][0:3],
)
ax0.text(
    400,
    120,
    str(np.round(v2.meta["restfrq"] / 1.0e9, 3)) + " GHz",
    fontsize=18,
    color=color_array[30][0:3],
)
ax0.text(
    400,
    140,
    str(np.round(v4.meta["restfrq"] / 1.0e9, 3)) + " GHz",
    fontsize=18,
    color=color_array[40][0:3],
)
ax0.text(
    400,
    160,
    str(np.round(v6.meta["restfrq"] / 1.0e9, 3)) + " GHz",
    fontsize=18,
    color=color_array[50][0:3],
)
...
if plot_vlasize:
    ...
    im0 = ax[0].imshow(Tb_mean_r1 / 1.0e6, origin=0, cmap="jet", interpolation="None")
    divider = make_axes_locatable(ax[0])
    cax = divider.append_axes("right", size="5%", pad=0.05)
    fig.colorbar(im0, cax=cax, label="T$_{B}$ (MK)")
    ax[0].set_yticks([0, 10, 20, 30])
    ax[0].set_yticklabels([freq[0], freq[10], freq[20], freq[30]])
    ax[1].plot(Tb_mean_r1[10] / 1.0e6, "o-", label="1.077 GHz")
    ax[1].legend()
    ...
    ax[2].legend()
    ax[2].set_xticks([0, 30, 60, 90, 120])
    ax[2].set_xticklabels(["18:44:00", "18:44:30", "18:45:00", "18:45:30", "18:46:00"])
    ax[2].set_xlabel("Time (HH:MM:SS UT)")
    ax[0].set_ylabel("Frequency (GHz)")
    ax[1].set_ylabel("T$_{B}$ (MK)")
    ax[2].set_ylabel("Source size (arcsec$^{2}$)")
    plt.show()
...
if plot_base_hmi_movie:
    for i in range(1, len(maphmir)):
        ...
        ax0 = f.add_subplot(111)
        cc = maphmir[i]
        xlaia = cc.center.Tx.value - 0.5 * int(cc.data.shape[0] / 2)
        xraia = cc.center.Tx.value + 0.5 * int(cc.data.shape[0] / 2)
        ylaia = cc.center.Ty.value - 0.5 * int(cc.data.shape[1] / 2)
        yraia = cc.center.Ty.value + 0.5 * int(cc.data.shape[0] / 2)
        p = cc.plot(
            extent=[xlaia, xraia, ylaia, yraia],
            aspect="auto",
            vmin=-80,
        ...
        ax0.set_xlim([-800, -730])
        ax0.set_ylim([230, 300])
        ax0.plot([-778, -778], [245, 268], "o", color="cyan")
...
if plot_base_movie:
    for i in range(len(allmapsb["aiab171"]["datab171"])):
        ...
        ax0.set_xlim([-800, -730])
        ax0.set_ylim([200, 270])
        ax0.plot([-778, -778], [245, 268], "o", color="k")
        ax0 = f.add_subplot(222)
        ...
        ax0.plot([-778, -778], [245, 268], "o", color="k")
        plt.savefig(
            "/media/rohit/VLA/20160409/pngs/base_zoom_aia_" + str("%03d" % i) + ".png"
        )
        plt.close()
...
if plot_euv1600_movie:
    for i in range(len(allmaps["aia1600"]["data1600"])):
        ...
        plt.ylim([50, 550])
        plt.savefig(
            "/media/rohit/VLA/20160409/pngs/aia1600_" + str("%03d" % i) + ".png"
        )
        plt.close()
...
for i in range(1):
    ...
    plt.text(401, 250, str(np.round(v1.meta["crval3"] / 1.0e9, 4)) + " GHz", color="r")
    ...
    plt.show()
    plt.savefig(
        "/media/rohit/VLA/20160409/pngs_50ms/aia131_contour_" + str("%03d" % i) + ".png"
    )
    plt.close()
...
for i in range(2399):
    ...
    p = cc.plot(axes=ax0, extent=[xlaia, xraia, ylaia, yraia], aspect="auto")
    lev1 = (
        (1.5e7 / dd.data.max()) * np.array([20, 30, 40, 50, 60, 70, 80, 90]) * u.percent
    )
    ...
    ax0.set_title(
        "AIA 171 $\AA$:" + timstr_171[tidx171[i]] + " VLA: " + timstr_vla[i] + " UT"
    )
    ax0.set_xlim([xlaia, xraia])
    ax0.set_ylim([ylaia, yraia])
    ax0.text(-1200, 0, str(np.round(dd.meta["crval3"] / 1.0e9, 4)) + " GHz", color="r")
    ax0.text(-1200, 50, "Contours: 3, 4.5, 6, 7.5, 9, 10, 12, 13 MK", color="yellow")
    # ax0.text(-1200,50,'Contours: 20%, 30%, 40%, 50%, 60%, 70%, 80%, 90%',color='yellow')
    ax1 = f.add_subplot(212)
    ...
    ax1.axvline(x=i * 0.05, color="k")
    ax1.set_xlabel("Time (sec)")
    ax1.set_ylabel("Median Amplitude")
    plt.savefig(
        "/media/rohit/VLA/20160409/pngs_50ms/aia131_contour_Tb_"
        + str("%03d" % i)
        + ".png"
    )
    plt.close()
...
for i in range(32):
    ...
    ax1.set_ylim(-30, 30)
    ax0.set_ylim(235, 275)
    ax0.legend()
    ax0.set_title(str(freq[i]) + " GHz")
    ax0.axhline(y=255, linestyle="--", color="black")
    ax2 = ax1.twinx()
    ax2.plot(x, maxTbi[i] / 1.0e6, "-", color="black")
    ax2.set_ylabel("T$_B$ (MK)")
    plt.savefig(
        "/media/rohit/VLA/20160409/spec_max/stokes_I_" + str("%04d" % i) + ".png"
    )
    f.clear()
    plt.close()
...
for i in range(1900):
    ...
    im0 = ax0.scatter(
        xcr90[i + 50].reshape(1, 32).mean(axis=0)[0:16],
        ycr90[i + 50].reshape(1, 32).mean(axis=0)[0:16],
        c=freq.reshape(1, 32).mean(axis=0)[0:16],
        s=40,
    ...
    ax0.set_xlim([-810, -750])
    ax0.set_ylim([220, 280])
    f.colorbar(im0, label="(RR) Frequency (GHz)")
    ax1 = f.add_subplot(212)
    ax1.plot(
        x,
        TbRRr1[i : i + 100, 0] / 1.0e6,
        "o-",
        markersize=5,
        label="Region 1 (0.994 GHz)",
    )
    ax1.plot(x[50], TbRRr1[i + 50][0] / 1.0e6, "o", color="red", markersize=10)
    ...
    ax1.legend(loc=2)
    ax1.axvline(x=x[50], color="k")
    ax1.set_ylim(0, 100)
    ax1.set_xlabel("Time (sec)")
    ax1.set_ylabel("$T_B$ (MK)")
    plt.savefig(
        "/media/rohit/VLA/20160409/pngs_50ms_spw0/centroid_llrr_"
        + str("%04d" % i)
        + ".png"
    )
    f.clear()
...
for i in range(2399, 6799):
    ...
    centx = np.hstack((np.array(qsxcr90), xcr90[1]))
    centy = np.hstack((np.array(qsycr90), ycr90[1]))
    f = plt.figure(figsize=(10, 15))
    cc = allmaps["aia171"]["map171"][tidx171[i]]
    xlaia = cc.center.Tx.value - 0.61 * int(cc.data.shape[0] / 2)
    xraia = cc.center.Tx.value + 0.61 * int(cc.data.shape[0] / 2)
    ...
    ax1 = f.add_subplot(212)
    ax1.plot(
        x, y[i : i + 100] / 1.0e6, "o-", markersize=5, label="Region 1 (0.994 GHz)"
    )
    ax1.plot(x[50], y[i + 50] / 1.0e6, "o", color="red", markersize=10)
    ax1.legend(loc=2)
    ax1.axvline(x=x[50], color="k")
    ax1.set_ylim(0, 100)
    ax1.set_xlabel("Time (sec)")
    ax1.set_ylabel("$T_B$ (MK)")
    plt.savefig(
        "/media/rohit/VLA/20160409/pngs_50ms_spw0_qs/centroid_llrr_"
        + str("%04d" % i)
        + ".png"
    )
    f.clear()
...
for i in range(1900):
    ...
    p = cc.plot(axes=ax0, extent=[xlaia, xraia, ylaia, yraia], aspect="auto")
    im0 = ax0.errorbar(
        xcLL90[i + 50].reshape(1, 32).mean(axis=0)[0:6].mean(),
        ycLL90[i + 50].reshape(1, 32).mean(axis=0)[0:6].mean(),
        xerr=xcLL90[i + 50].reshape(1, 32).mean(axis=0)[0:6].std(),
        yerr=ycLL90[i + 50].reshape(1, 32).mean(axis=0)[0:6].std(),
        color="blue",
    )
    im0 = ax0.errorbar(
        xcRR90[i + 50].reshape(1, 32).mean(axis=0)[0:16].mean(),
        ycRR90[i + 50].reshape(1, 32).mean(axis=0)[0:16].mean(),
        xerr=xcRR90[i + 50].reshape(1, 32).mean(axis=0)[0:16].std(),
        yerr=ycRR90[i + 50].reshape(1, 32).mean(axis=0)[0:16].std(),
        color="magenta",
    )
    ax0.set_title(
        "AIA 171 $\AA$:" + timstr_171[tidx171[i]] + " VLA: " + timstr_vla[i] + " UT"
    ...
    ax1.set_ylabel("$T_B$ (MK)")
    plt.savefig(
        "/media/rohit/VLA/20160409/pngs_50ms_spw0/centroid_llrr1_"
        + str("%04d" % i)
        + ".png"
    )
    f.clear()
...
for i in range(1900):
    ...
    # ax0.set_xlim([xlaia,xraia]);ax0.set_ylim([ylaia,yraia])
    # ax0.set_xlim([-1000,-700]);ax0.set_ylim([50,350])
    ax0.set_xlim([-830, -730])
    ax0.set_ylim([200, 300])
    f.colorbar(im0, label="Frequency (GHz)")
    ax1 = f.add_subplot(212)
    ax1.plot(
        x,
        Tb_r[0][i : i + 100] / 1.0e6,
        "o-",
        markersize=5,
        label="Region 1 (0.994 GHz)",
    )
    ax1.plot(x[50], Tb_r[0][i + 50] / 1.0e6, "o", color="red", markersize=10)
    # ax1.plot(x,Tb_r[1]/1.e6,'-',label='Region 2')
    # ax1.plot(x,Tb_r[2]/1.e6,'-',label='Region 3')
    # ax1.plot(x,Tb_r[3]/1.e6,'-',label='Region 4')
    # ax1.plot(x,Tb_r[4]/1.e6,'-',label='Region 5')
    ...
    ax1.set_ylabel("$T_B$ (MK)")
    plt.savefig(
        "/media/rohit/VLA/20160409/pngs_50ms_spw0/centroid_" + str("%04d" % i) + ".png"
    )
    f.clear()
    plt.close()
...
for i in range(1900):
    ...
    yrvla = dd.center.Ty.value + 2.0 * int(dd.data.shape[0] / 2)
    dd.draw_contours(
        levels=lev1, colors="black", linewidths=1, extent=[xlvla, xrvla, ylvla, yrvla]
    )  # [xlpix,xrpix,ylpix,yrpix])
    ax0.set_title(
        "AIA 171 $\AA$:" + timstr_171[tidx171[i]] + " VLA: " + timstr_vla[i] + " UT"
    )
    ...
    ax0.set_ylim([200, 300])
    ax0.text(
        -830, 210, str(np.round(dd.meta["crval3"] / 1.0e9, 4)) + " GHz", color="white"
    )
    ...
    # ax0.add_patch(patches.Rectangle((-831,175),40,40,linewidth=5,edgecolor='magenta',facecolor='none'))
    ax1 = f.add_subplot(212)
    ax1.plot(x, Tb_r[0][i : i + 100] / 1.0e6, "o-", markersize=5, label="Region 1")
    ax1.plot(x[50], Tb_r[0][i + 50] / 1.0e6, "o", color="red", markersize=10)
    # ax1.plot(x,Tb_r[1]/1.e6,'-',label='Region 2')
    # ax1.plot(x,Tb_r[2]/1.e6,'-',label='Region 3')
    # ax1.plot(x,Tb_r[3]/1.e6,'-',label='Region 4')
    ...
    ax1.set_ylabel("$T_B$ (MK)")
    plt.savefig(
        "/media/rohit/VLA/20160409/pngs_50ms_spw0/regions_contour_base_"
        + str("%04d" % i)
        + ".png"
    )
    f.clear()
...
for i in range(2399):
    ...
    ax1.set_ylabel("Median Amplitude")
    plt.savefig(
        "/media/rohit/VLA/20160409/pngs_50ms/aia171_contour_" + str("%03d" % i) + ".png"
    )
    plt.close()
...
for i in range(2399):
    ...
    xl = SkyCoord(
        dd.center.Tx.value - 0.5 * (2.0 / 0.6) * dd.data.shape[0],
        dd.center.Ty.value - 0.5 * (2.0 / 0.6) * dd.data.shape[1],
        frame=cc.coordinate_frame,
        unit=(u.arcsec, u.arcsec),
    )
    xlpix = cc.world_to_pixel(xl).x.value
    ylpix = cc.world_to_pixel(xl).y.value
    yr = SkyCoord(
        dd.center.Tx.value + 0.5 * (2.0 / 0.6) * dd.data.shape[0],
        dd.center.Ty.value + 0.5 * (2.0 / 0.6) * dd.data.shape[1],
        frame=cc.coordinate_frame,
    ...
    xrpix = cc.world_to_pixel(yr).x.value
    yrpix = cc.world_to_pixel(yr).y.value
    dd.draw_contours(
        levels=lev1, colors="r", linewidths=3, extent=[xlpix, xrpix, ylpix, yrpix]
    )
    plt.text(401, 250, str(np.round(dd.meta["crval3"] / 1.0e9, 4)) + " GHz", color="r")
    plt.text(401, 50, "Contours: 0.6,0.75,0.9,1.0,1.2,1.3 MK", color="yellow")
    plt.title(
        "AIA 1600 $\AA$:" + timstr_1600[tidx131[i]] + " VLA: " + timstr_vla[i] + " UT"
    )
    plt.xlim([400, 900])
    plt.ylim([50, 550])
    plt.savefig(
        "/media/rohit/VLA/20160409/pngs_50ms/aia1600_contour_"
        + str("%03d" % i)
        + ".png"
    )
    plt.close()
...
for i in range(1):
    i = 1400
    compmap = Map(
        allmaps["aia171"]["map171"][tidx171[i]],
        allmaps["vla"]["mapvla"][i],
        composite=True,
    )
...
for i in range(119):
    ...
    lev1 = (3.0e7 / v1.data.max()) * np.array([50, 60, 70, 90]) * u.percent
    lev2 = (3.0e7 / v2.data.max()) * np.array([50, 60, 70, 90]) * u.percent
    lev3 = (3.0e7 / v3.data.max()) * np.array([50, 60, 70, 90]) * u.percent
    v1.draw_contours(levels=lev1, colors="r", linewidths=3, extent=p.get_extent())
    v2.draw_contours(levels=lev2, colors="g", linewidths=3, extent=p.get_extent())
    v3.draw_contours(levels=lev3, colors="b", linewidths=3, extent=p.get_extent())
    plt.xlim([400, 900])
    plt.ylim([50, 550])
    plt.text(401, 250, str(np.round(v1.meta["crval3"] / 1.0e9, 4)) + " GHz", color="r")
    plt.text(401, 200, str(np.round(v2.meta["crval3"] / 1.0e9, 4)) + " GHz", color="g")
    plt.text(401, 150, str(np.round(v3.meta["crval3"] / 1.0e9, 4)) + " GHz", color="b")
    plt.text(401, 50, "Contours: 15,18,21,27 MK", color="yellow")
    plt.title("VLA: " + v1.meta["date-obs"])
    plt.savefig(
        "/media/rohit/VLA/20160409/pngs/aia1600_contour_" + str("%03d" % i) + ".png"
    )
    plt.close()
...
compmap.set_plot_settings(
    1, {"cmap": "gray", "norm": mpl.colors.Normalize(vmin=-1.0, vmax=1.0), "origin": 0}
)
compmap.plot()
plt.show()
...
if plot_full_ds:
    ...
    ax1.imshow(
        ...
        aspect="auto",
        cmap="jet",
        extent=[time[0], 7.5, freq[0], 1.122],
        interpolation=None,
        origin=0,
        vmin=-3,
        vmax=1,
