...
def do_bdsf(imagepath,output_model,idx_f):
    ...
    img_bdsf = bdsf.process_image(out_imagepath, beam=(10/3600.,10/3600.,0),thresh_pix=3,thresh_isl=3,thresh='hard')
...
for k in range(nk):
        ...
        if((k>5) & (do_point_source_det==1)):
            print('Doing pybdf.....')
            output_model='/scratch/snx3000/rsharma/pybdsf_tests/pybdsf_skymodel_'+kk+'.txt'
	...
	#---------------- Do Point Source Simulation-------------------------
        ...
        iono_fits='/scratch/snx3000/mibianco/output_sdc3/dataLC_256_train_090523/atmo/screen_4h_i0_ch'+str(idx_f)+'.fits'
	...
	#--------------- Subtractig the Point Source Vis
        ...
        filename_res='/scratch/snx3000/rsharma/pybdsf_tests/residual_sdc3point_ch750_'+kk+'.MS'
        write_ms(filename_res,ms_new_res,skadc_uvw,130816)
        ...
        if(get_uvlim):
        	...
        	filename_res1='/scratch/snx3000/rsharma/pybdsf_tests/residual_sdc3point_ch750_uvlim'+str(uvlim)+'_'+kk+'.MS'
        	...
        	res_vis1=res_read1.read_vis(start_row=0,start_channel=0,num_channels=1,num_baselines=num_baselines_)
        	imager = oskar.Imager()
        	imager.fov_deg = 4             # 0.1 degrees across.
        	imager.image_size = 2048          # 256 pixels across.
        	imager.set_output_root(imagepath)
...
img0=fits.open('res_image_ch750_00_I.fits');d0=img0[0].data[0]
img1=fits.open('image_ch750_01_I.fits');d1=img1[0].data[0]
img2=fits.open('image_ch750_02_I.fits');d2=img2[0].data[0]
...
ax0.set_xlabel('UV distance (m)');ax1.set_xlabel('UV distance (m)');ax2.set_xlabel('UV distance (m)')
ax0.set_ylabel('Amplitude')
ax0.set_title('SKA');ax1.set_title('Residual');ax2.set_title('Point')
plt.show()
...
ax0.imshow(ska_image,origin='lower',cmap='coolwarm',extent=[-2,2,-32,-28],aspect='auto',vmin=-0.1,vmax=0.1)
ax0.set_title('SKA Data')
im1=ax1.imshow(res_image1,origin='lower',cmap='coolwarm',extent=[-2,2,-32,-28],aspect='auto',vmin=-0.1,vmax=0.1)
ax1.set_title('Residual')
ax2.imshow(point_image,origin='lower',cmap='coolwarm',extent=[-2,2,-32,-28],aspect='auto',vmin=-0.1,vmax=0.1)
ax2.set_title('Point Source')
ax3.imshow(res_image_masked,origin='lower',extent=[-2,2,-32,-28],cmap='coolwarm',aspect='auto',vmin=-0.1,vmax=0.1)
ax3.set_title('Residual Masked 5-sigma')
divider = make_axes_locatable(ax1)
cax = divider.append_axes('right', size='5%', pad=0.05)
f.colorbar(im1, cax=cax, orientation='vertical')
ax3.set_xlabel('RA (deg)');ax2.set_xlabel('RA (deg)');ax2.set_ylabel('Dec (deg)');ax0.set_ylabel('Dec (deg)')
plt.show()
