## SCRIPT TO DO VISIBILITY SUBSTRACTION AND SELFCAL ON CALIBRATED VLA DATA
...
MOMMS='20130423T204521-204522.L.50ms.cal.ms'
do_subvs=0 # Generates visibility subtracted data for a given timerange
if(do_subvs):
    output_timerange='20:40:54~20:40:55' # Time range from which subtraction will be done
    outname='20130423T204054-204055.L.50ms.cal.ms'
    subtime='21:23:00.0~21:26:00.0' # Time needed to be subtracted
    subvs(vis=MOMMS,outputvis=outname,timerange=output_timerange,subtime1=subtime,mode="linear",splitsel=True,overwrite=False)
os.system('rm -rf 20130423T204521-204522.L.50ms.cal.ms')
os.system('cp -r 20130423T204521-204522.L.50ms.cal.copy.ms 20130423T204521-204522.L.50ms.cal.ms')

########## SELFCAL PROCEDURE BELOW ####################

# INPUT PARAMETERS
MS_='20130423T204521-204522.L.50ms.cal.ms'
DIR='selfsol' # Directory into which all solutions will be stored.
prefix=DIR+'/fsun.selfcal_soln' # Naming convention for the filenames
imagename_final='20130423T204521-204521_final' # Name of the final images
chan_final='50~55' # Spectral window selection for final images
timerange_final='20:45:21~20:45:22' # Time range for final images
antennas_=''#'ea01,ea03,ea06,ea07,ea08,ea09,ea17,ea18,ea21,ea25,ea26,ea27' 
refant_=''# Reference antenna
solar_centre='J2000 02h05m40.74 +12d44m01.6' # Optional to use
phase_centre='J2000 02h04m51.211 12d43m12.47' # From Yingjie script
masks_=['mask0.mask'] # Select a region to mask in the images
initial_model_L_band='' # Provide initial solution, if available.

# STANDARD PARAMETERS 
spw_list=['6'] # List of all the spectral windows required to do selfcal
chanlist=['3~60']*9 # List of channels for making models
gainchan='28~32' # Choose channels for gaincal 
interactive=False # For making models
timerange_='20:45:21~20:45:22' # Specify time range for selfcal
modes=['p','p','a','p','p','p'] # list of phase modes
snr_=[2,3,3,3,3,3,3] # list of snr for different modes
caltable_all=[0]*len(spw_list) # list of selfcal tables
MS=[0]*(len(modes)+1) # String for new MS from each selfcal iterations
MS[0]=MS_ # Initialise first MS
#niter_scale_factor=np.ones(len(spw_list)) # Optional, default: 1
#niter_scale_factor[1]=1.0 # Optional, default: 1
niter_base=np.array([100,200,400,400,400,400,400,400,400]) # List of iterations for making models, if interactive false
uvrange_=['','','','','','','','','',''] # List of UV ranges for each selfcal iteration
stokes_='I' # Stokes parameter
imsize_=[256,256] # Image sizes
cell_=['2.5arcsec','2.5arcsec'] # Cell sizes

# SCRIPT'S TAG 
first_clearcal=1 # Do clearcal before starting selfcal iteration
make_model=1 # Make model in selfcal
apply_model=1 # Applycal model in selfcal
use_initial_model=0 # Optional, if you want to use some initial model
...
for spw_ in spw_list:
    ...
    for step in range(len(modes)):
        ...
        if(apply_cal_type1):
            ...
            print 'Applying solutions....',[caltable_],'..to '+MS[step]
        ...
        if(make_image):
            ...
            clean(vis=MSI,imagename=image_f[step], \
                    outlierfile="",field="",spw=spw_+':'+chan_final,selectdata=True,timerange="", \
                    uvrange="",antenna=antennas_,scan="",observation="",intent="",mode="mfs",resmooth=False, \
                    ...
                    interpolation="linear",niter=500,gain=0.1,threshold="0.0mJy",psfmode="hogbom",imagermode="csclean", \
    ...
    if(do_clearcal_momms):
        clearcal(MOMMS,spw=spw_)
    if(apply_cal_momms):
        caltable_apply=caltable_all[ss]#list(itertools.chain(*caltable_all))
        print 'Applying solutions to MOM MS....',caltable_apply
        applycal(vis=MOMMS,field="",spw=spw_+':'+chan_final,intent="",selectdata=True,timerange="",uvrange="",antenna="", \
