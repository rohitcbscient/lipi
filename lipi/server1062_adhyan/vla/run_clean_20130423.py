...
#MSNAME='20130423T2050-2100.L.50ms.cal.ms'
#MSNAME='20130423T2000-2127.L.50ms.cal.ms'
#MSNAME='/nas08-data02/vladata/20130423/L-Band/20130423T2044-2046.L.50ms.cal_bgsub.sp.ms'
MSNAME='/nas08-data02/vladata/20130423/L-Band/20130423T2030-2050.L.50ms.cal.ms'
#MSNAME='20130423T2015-2030.L.50ms.cal.ms'
#MSNAME='20130423T2000-2015.L.50ms.cal.ms'
#MSNAME='20130423T2015-2030.L.50ms.selfcal.ms'
#MSNAME='20130423T202641.5-202642.5.L.50ms.cal.slfcal.4.ms'
#MSNAME='20130423T202641.5-202642.5.L.50ms.cal.ms'
#MSNAME='20130423.L.50ms.selfcal.ms'
###### im.defineimage 
IMSIZE=256
CELLSIZE='2.0arcsec'
STOKES='LL'
...
first_time='20:45:25' # end time '20:46:42.5' del t = 17.5s
i=0
...
#TIME_LIST=TIME_LIST[2400:2820]
#time_pair=time_pair[2400:2820]
if(FBAND=='L'):
...
k=1#494*3
#for i in range(1):
#    time_pair[i]='20:56:05.5~20:56:06.5'
for i in range(TIME_LIST.shape[0]-2):
    ss=0
    for s in [spw_list[-8]]:  # spw_list[51] RSTN 1.415GHz, spw_list[88] RSTN 2.695GHz
    #for s in spw_list[32*k:32*(k+1)]:
            sname=s.split(':')[0]+'_'+s.split(':')[1].split('~')[0]+'-'+s.split(':')[1].split('~')[1]
            tname=time_pair[i].split('~')[0]+'-'+time_pair[i].split('~')[1]
            #imagename[i]='selected_events/'+NAME+'.spw.'+sname+'.time.'+tname
            #imagename[i]='images_LL/'+NAME+'.spw.'+sname+'.time.'+tname
            imagename[i]='sub_spikes/images_LL_DC/'+NAME.split('/')[-1]+'.spw.'+sname+'.time.'+tname
